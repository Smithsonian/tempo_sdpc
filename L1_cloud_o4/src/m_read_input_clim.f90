...
contains
   ...
   use m_vars, only: geos_np, gmetadata, nlayers,&
  ...
  real (kind=8) :: t_beg, t_end, hour_beg, hour_end, tai93_offset

!  write(*,*) 'Preapre to read GEOS-CF for orbit'
  nlayers = geos_np

  tai93_offset=0.0d0
  t_beg = minval(rad_time, rad_time /= r8_missval) - tai93_offset
  t_end = maxval(rad_time, rad_time /= r8_missval) - tai93_offset
...
!
   ...
   integer :: ix, it, iz, nx, nt, kk, nz, errstat
   integer :: thisyear, thismonth, thisday
  ...
  ! loop through pixels
   do it = 1, nt
     ...
     call tio_f_taix_time_to_utc_caldate(rad_time(it)-tai93_offset, &

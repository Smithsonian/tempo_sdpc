...
!  SmPx_reader_class module to obtain the geolocation and data fields from an
!  OMI L1B data file in HE4 format and computes a cloud mask based on 
!  spatial homogeneity
!
...
!
!  The use of Fortran 90 keywords allows subsets of the geolocation and data
!  fields to be obtained.  For example, if just the small pixel values and
!  wavelengths for one "scan" line are needed, then the L1Br_getSIGline
...
!  CHARACTER (LEN = 200) :: valname, valname2
...
! obtain sizes of dimensions defined in swath
   ...
   DO iLine = 0, nTimes-1
      ...
      !status = L1Br_getSIGline( blk, iLine, NumberSmallPixelColumns_k=nPix, &
      !                          SmallPixelSignal_k=smvaluesL, &
      !                          SmallPixelWavelength_k=wavelengthL)!, quality_flagL )
      status = L1Br_getDATAline( blk, iLine, nPix, &
                                Data_k=smvaluesL, &
...
! science data processing
      ...
      do iTrack=1, nXtrack
       !if (iprt >= 7) print *,'cloud_mask: proc. data from track ',iTrack
        ...
        if (indc .eq. 1) then
         !smpx_stddev(iTrack,iLine+1)=sigma(smvaluesL(iTrack,1:nPix), &
         !   avg1=smpx_mean(iTrack,iLine+1))
           ...
           if (nPix > 2) then
            ...
            smpx_mean(iTrack,iLine+1) = d(1)
             if (iprt >= 8) then
               ...
               write(6,101) r1_ri(1:npix-1,1)/smvaluesL(iTrack,order(1))
               write(6,101) r1_ri(1:npix-1,2)/smvaluesL2(iTrack,order(1))
               write(6,102) d(1:npix-1)/sum(d(1:npix))*100. 
               write(6,101) A(1:npix-1,1)
               write(6,101) A(1:npix-1,2)
 101        format(5e12.3)
 102        format(5f12.3)
             endif ! iprt 
            endif ! npix > 2
           else ! use only vis
         ! maxs=find(smvaluesL(iTrack,1:nPix) == &
         !   maxval(smvaluesL(iTrack,1:nPix)),iprt=0)
!          smpx_stddev(iTrack,iLine+1)=abs(smvaluesL(iTrack,mins) - &
!             smvaluesL(iTrack,maxs)) / &
            smpx_stddev(iTrack,iLine+1)=abs(smvaluesL(iTrack,1)- &
               smvaluesL(iTrack,nPix)) / &
               smpx_mean(iTrack,iLine+1)
!          if (iprt == 8) then
!           write(6,100) smvaluesL(iTrack,1), smvaluesL(iTrack,nPix)
!100        format(2e25.8)
!          endif
         ...
         !if (smpx_stddev(iTrack,iLine+1) >= stddev_thresh) then
           cloud_mask(iTrack,iLine+1)=1
         elseif (smpx_stddev(iTrack,iLine+1) > 0) then
           cloud_mask(iTrack,iLine+1)=0
         endif
         !if (iprt >= 7) then
         ! write(6,*) "nPix smvaluesL"
         ! write(6,*) nPix, smvaluesL(itrack,1:nPix)
         ! write(6,*) "wavelenghtL"
         ! write(6,*) wavelengthL(itrack,1:nPix)
         ! write(6,*) "std, cloudmask"
         ! write(6,*) smpx_stddev(iTrack,iLine+1), cloud_mask(iTrack,iLine+1)
         !endif
         !if (iprt >= 3) then
         ! write(17,*) npix,smpx_stddev(iTrack,iLine+1), cloud_mask(iTrack,iLine+1)
         !endif
       endif ! if npix found
      enddo ! loop over Xtrack
      !endif ! not noret
 
! write(6,*) "quality flag"
! write(6,*) quality_flagL(1:nXtrack)
! write(6,*) 

...
!**************************************************************************
 ...
 if (iprt >= 3) then
  ...
  if (iter == 0) then
    ...
    write(6,101) lat(ip+1,iLine), lon(ip+1,iLine), sza(ip,iLine), chlcl(ip)
    print *,'x_fg '
    write (6,100) x_fg
  endif
  print *,'x, iter= ',iter
  ...
  if (iprt >= 7 .and. iter == 1) then
   ...
   if (cld_frac > cld_frac_min .and. cld_frac < 1.0 .and. &
    get_cloud_frac .and. iprt >= 7) then
     ...
     write(6,100) rad_clr
     print *,'ring_clr'
     write(6,100) ring_clr
   endif
   print *,'ring_cld'
   write(6,100) ring_cld
    if (add_oc .and. ((.not. ret_chl .and. iter == 0) .or. ret_chl)) then
     if (ret_chl .and. reflec < 0.2) then
       ...
       write(6,100) ring_ocs(ic1,:)
       print *,'ring_ocs 2 ',ic2
       write(6,100) ring_ocs(ic2,:)
       print *,'ring_oc'
       write(6,100) ring_oc
       print *,'rad_clr_oc'
       write(6,100) rad_clr_oc
       if (ret_chl) then
         print *,'h_chl'
         write(6,100) h(:,nst-nterms-2-nsh)
       endif
       !stop
     endif
    endif
   ...
   write(6,100) rad_clds(ix1,:)
   print *,'rad_clds 2 ',ix2
   write(6,100) rad_clds(ix2,:)
   print *,'ring_clds 1 '
   write(6,100) ring_clds(ix1,:)
   print *,'ring_clds 2 '
   write(6,100) ring_clds(ix2,:)
   print *,'h(0,*)'
   write(6,100) h(:,0)
   if (ret_chl) then
     print *,'h(chloro,*)'
     write(6,101) h(:,nst-nterms-2-nsh)
   endif
   if (squeeze) then
     print *,'h(1,*)'
     write(6,100) h(:,1)

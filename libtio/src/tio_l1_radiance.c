   ...
   /* radiance error */
     {
        ...
        dims[1] = dim_table->xtrack.id;
        dims[2] = dim_table->channel.id;
        if (-1 == _pTIO_define_var_with_text_attrs (grp, TEMPO_VAR_RADIANCE_ERROR, NC_FLOAT, 3, dims, radiance_error_attrs, &varid))
          return -1;
        if (NC_NOERR != (status = nc_put_att (grp, varid, _FillValue, NC_FLOAT, 1, &radiance_error_fill)))
          {
             Tell_verror (TELL_IO_WRITE_ERROR, "writing %s fill value to grp=%d (%s)",

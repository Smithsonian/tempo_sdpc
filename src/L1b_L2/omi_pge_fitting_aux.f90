...
CONTAINS
  ...
  SUBROUTINE omi_create_solcomp_irradiance ( nxt )
    ...
    ! -----------------------------------------------------------
    swvl = fit_winwav_lim(1) ; ewvl = fit_winwav_lim(N_FIT_WINWAV)
...
!UNUSED!   SUBROUTINE common_mode_save_vars (                                   &
!UNUSED!       frrr, frrr_sav, frrf, frrf_sav, nflr, nflr_sav, nflf, nflf_sav, &
!UNUSED!       fl, fl_sav, ll, ll_sav )
!UNUSED! 
!UNUSED!     USE OMSAO_precision_module, ONLY: i4
!UNUSED!     IMPLICIT NONE
!UNUSED! 
!UNUSED!     ! -----------------------------------------------------------------
!UNUSED!     ! The argument list is admittedly cryptic, but the purpose of this
!UNUSED!     ! subroutine is decidedly simple: Copy the values of any variable
!UNUSED!     ! XXX to XXX_SAVE.
!UNUSED!     !
!UNUSED!     ! We have to do this for a range of settings when dealing with
!UNUSED!     ! on-line computation of the common mode spectrum.
!UNUSED!     ! -----------------------------------------------------------------
!UNUSED! 
!UNUSED!     INTEGER (KIND=i4), INTENT (IN)  :: frrr, frrf, nflr, nflf, fl, ll
!UNUSED!     INTEGER (KIND=i4), INTENT (OUT) :: frrr_sav, frrf_sav, nflr_sav, nflf_sav, fl_sav, ll_sav
!UNUSED! 
!UNUSED!     frrr_sav = frrr
!UNUSED!     frrf_sav = frrf
!UNUSED!     nflr_sav = nflr
!UNUSED!     nflf_sav = nflf
!UNUSED!     fl_sav   = fl
!UNUSED!     ll_sav   = ll
!UNUSED! 
!UNUSED!     RETURN
!UNUSED!   END SUBROUTINE common_mode_save_vars

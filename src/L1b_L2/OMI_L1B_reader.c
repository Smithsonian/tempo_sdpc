...
#if PERFORM_TIMING
   static double cumsecs = 0;
   static unsigned int ncalls;
   struct timeval tv;
   double tstart, tstop;

   (void) gettimeofday (&tv, NULL); tstart = tv.tv_sec + 1e-6*tv.tv_usec;
#endif
   *fldflag = 0;
...
#if PERFORM_TIMING
   (void) gettimeofday (&tv, NULL); tstop = tv.tv_sec + 1e-6*tv.tv_usec;
   cumsecs += tstop - tstart;
   ncalls++;
   (void) fprintf (stdout, "OMI_Get_L1B_Data_Block %s: cumsecs=%g, ncalls=%u, secs/call=%g\n",
		   fieldname, cumsecs, ncalls, cumsecs/ncalls);
#endif

...
def select_matching_radref (c, window_days, window_hours, info):
    sec_per_day = 86400.0
    sec_per_hour = 3600.0
    tx = info["tstart"]
    htx = tx % sec_per_day
    ...
    subst = {'beg':tx-tmax, 'end':tx-sec_per_day, }
    cmd = "select path,tstart from 'RADREF_L1' where tstart >= :beg and tend <= :end order by tstart desc;"
    c.execute(cmd, subst)
    ...
    for row in c:
        tstart = row[1]
        t_delta = abs(tx - tstart) / sec_per_day
        h_delta = abs(htx - (tstart % sec_per_day)) / sec_per_day
...
def main():
    ...
    parser.add_argument ('--days',default=2.0, type=float,
                         help="Acceptable offset in days")
    parser.add_argument ('--hours', default=2.0, type=float,
    ...
    with connect_database (db_path) as conn:
        c = conn.cursor()
        try:
            radref = select_matching_radref (c, args.days, args.hours, rad_info)
        except:
            radref = ""

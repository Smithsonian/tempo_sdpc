    ...
    CONTAINS
       FUNCTION L2_createFile( L2_file_LUN, L2_filename ) RESULT( status )
         ...
         IF( SW_fileid == -1 ) THEN
            ...
            ierr = OMI_SMF_setmsg( OZT_E_HDFEOS, msg, "L2_createFile", zero )
            status = OZT_E_FAILURE
            RETURN 
         ELSE
            WRITE( msg,'(A)' ) "he5_swopen:"// TRIM(L2_filename) //&
                               " created succesfully."
            ierr = OMI_SMF_setmsg( OZT_S_SUCCESS, msg, "L2_createFile", &
                                   four )
         ENDIF
       ...
       FUNCTION L2_setupSwath( L2_filename, dimList, dims, swath_name_LUN, &
         ...
         IF( status /= PGS_S_SUCCESS ) THEN
            WRITE( msg,'(A,I9)' ) "get swathname from PCF file at LUN =", &
                                 swath_name_LUN
         ...
         ELSE
            WRITE( msg,'(A)' ) "he5_swcreate:"// TRIM(L2_swathname) //&
                               ...
                               TRIM(L2_filename)
            ierr = OMI_SMF_setmsg( OZT_S_SUCCESS, msg, "L2_setupSwath", &
                                   four )
         ENDIF
         ...
         DO id = 1, nDims
            ...
            IF( status == -1 ) THEN
               ...
               ierr = OMI_SMF_setmsg( OZT_E_HDFEOS, msg, "L2_setupSwath", &
                                      zero )
               status = OZT_E_FAILURE
               RETURN
            ENDIF
       ...
       FUNCTION L2_newBlockW( this, filename, swathname, &
         ...
         IF( this%nFields == 0 .OR. this%nFields > NFLDS_MAX ) THEN
            WRITE( msg, '(A,I2,A)' ) "number of fields =", this%nFields, &
                                     " out of range."
            ierr = OMI_SMF_setmsg( OZT_E_INPUT, msg, "L2_newBlockW", zero )
            status = OZT_E_FAILURE
            RETURN
         ENDIF
         ...
         DO id = 1, this%nFields
           ...
           IF( this%elmSize(id) <= 0 ) THEN
              WRITE( msg,'(A,I9,A)' ) "datatype: ", fieldList(id)%datatype, &
         ...
         SELECT CASE( df%datatype )
           CASE ( HE5T_NATIVE_INT8 )
             ...
             status = he5_swwrlattr( SW_id, df%name, "_FillValue", &
                                     HE5T_NATIVE_UINT8, n1, fill_int8 )      
           ...
           CASE ( HE5T_NATIVE_DOUBLE )
             ...
             ENDIF
           CASE DEFAULT 
            ierr = OMI_SMF_setmsg( OZT_W_GENERAL, "Valid range not set for "// &

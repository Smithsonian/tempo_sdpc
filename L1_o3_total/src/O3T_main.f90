...
PROGRAM O3T_mainNVAdj
  ...
  errstat = O3T_nval_setup( OMTO3_NVAL_LUN, nvRRS  )
  IF( errstat /= 0 ) THEN
    WRITE( msg,'(A)' ) "Read NVAL LUT failed, aborting."
    call tell_error(tell_io_read_error, msg, errstat)
    stop 1
  ENDIF
  errstat = O3T_getLambdaSet()
  if (errstat /= 0) then
    call tell_error(tell_runtime_error, "O3T_getLambdaSet failed", errstat)
    stop 1
  endif

  !! read in DNDX table
  errstat = O3T_dndx_setup( wl_cutoff )
  IF( errstat /= 0 ) THEN
    WRITE( msg,'(A)' ) "Read DNDX LUT failed, aborting."
    call tell_error(tell_io_read_error, msg, errstat)
    stop 1
  ENDIF

  !! calculate common part of the coefficient for interpolation
  errstat = O3T_lpoly_cden( LUT_cden_blk )
  IF( errstat /= 0 ) THEN
    WRITE( msg,'(A)' ) "O3T_lpoly_cden failed to set coefficient, aborting."
  ...
  if (use_tio_in) then
    ...
    if (status /= PGS_S_SUCCESS ) then
      call tell_error (tell_runtime_error, "reading irradiance file name", &
           errstat)
      stop 1
    endif
    ...
    IF( numfiles /= 1 ) THEN
      ...
      call tell_error(tell_invalid_parm_error, msg, errstat)
      stop 1
    ENDIF
    ...
    ENDIF
    IF( irr_error > 0 ) THEN
      WRITE( msg,'(A)' ) "One or more MeasurementQualityflags was set "// &
           "to error in irradiance file:"// TRIM(IRR_filename) // &
           ". Alternative file should be staged."
      call tell_error(tell_runtime_error, msg, errstat)
      stop 1
    END IF
  ...
  if (use_tio_in) then
    ...
    IF( status /= OZT_S_SUCCESS ) THEN
      WRITE( msg,'(A)' ) "O3T_getIRR failed, aborting."
      call tell_error(tell_runtime_error, msg, errstat)
      stop 1
    END IF
  endif
  ...
  if (use_tio_in) then
    ...
    if (status /= PGS_S_SUCCESS ) then
      call tell_error (tell_runtime_error, "reading radiance file name", &
           errstat)
      stop 1
    endif
  else
  ...
  if (use_tio_in) then
    ...
    IF( status /= OZT_S_SUCCESS ) THEN
      WRITE( msg,'(A)' ) "O3T_initRAD failed."
      call tell_error (tell_runtime_error, msg, errstat)
      stop 1
    END IF
  endif
  ...
  ! Read anomaly flags
  if (have_omi_data) then
    errstat= GET_ANOMFLG_3(year,jday,anomflg_3)
    if(errstat /= 0) then
      call tell_error(tell_runtime_error, "GET_ANOMFLG_3 failed", errstat)
      stop 1
    endif
    ...
    ALLOCATE(wl_cor(nWvc), &
         iwlSub(nWvc), &
         swpcr(nWvc,nXtc), stat=errstat)
    if (errstat /= 0) then
      call tell_error (tell_malloc_error, "O3T_main: malloc failed", errstat)
      stop 1
    endif
    errstat = OmiNvalueCorr(orbit_number, wl_cor, swpcr )
  endif

  !! read in climatology data
  errstat = O3T_apriori_rd()
  IF( errstat /= 0 ) THEN
  ...
  if (use_tio_out) then
    ...
    IF( status /= PGS_S_SUCCESS) THEN
      call tell_error (tell_runtime_error, &
           "reading L2 file name from PCF file", errstat)
      stop 1
    endif
  ...
  if (use_he5_out) then
    ...
    IF( status /= OZT_S_SUCCESS ) THEN
      call tell_error (tell_runtime_error, "OMI_setupL2swath failed", errstat)
      stop 1
    END IF
    ...
    IF( status /= OZT_S_SUCCESS ) THEN
      call tell_error (tell_runtime_error, "L2_defSWdatafields failed", &
           errstat)
      stop 1
    END IF
    ...
    IF( status /= OZT_S_SUCCESS ) THEN
      call tell_error (tell_runtime_error, "L2_newBlockW failed", &
           errstat)
      stop 1
    END IF
  ...
  DO iwl = 1, nWvc
    ...
    IF( il == 0 .OR. il == nwl_com ) THEN
      ...
      call tell_error (tell_runtime_error, "wl_cor outside wl_com range", &
           errstat)
      stop 1
    ELSE
      ih = il+1
      ...
      IF( ih /= il ) THEN
        call tell_error (tell_runtime_error, "wl_cor not matched in wl_com", &
             errstat)
        stop 1
      ENDIF
    ENDIF
  ...
  if (use_he5_out) then
    ...
    IF( status /= OZT_S_SUCCESS ) THEN
      call tell_error (tell_runtime_error, "L2_defSWgeofields failed", &
           errstat)
      stop 1
    END IF
    ...
    status = L2_newBlockW( geoblk, OMTO3_fn, OMTO3_swathname, &
         ...
         nLinesPerWrite )
    IF( status /= OZT_S_SUCCESS ) THEN
      call tell_error (tell_runtime_error, "L2_newBlockW for Geo failed", &
           errstat)
      stop 1
    END IF
    ...
    status = L2_newBlockW( datablk, OMTO3_fn, OMTO3_swathname, &
         ...
         nLinesPerWrite )
    IF( status /= OZT_S_SUCCESS ) THEN
      call tell_error (tell_runtime_error, "L2_newBlockW for data failed", &
           errstat)
      stop 1
    END IF
  endif ! end if (use_he5_out)
  ...
  DO iLine = iLine_b, iLine_e
    ...
    !! wavelength grid.
    errstat = L1Bri_interpWL( radWavelength, radQAflags, wl_com, &
...
!      if (.NOT. skipit .AND. errstat == 0) then
      ...
      IF( skipit .OR. errstat /= 0  ) THEN
        ...
        CYCLE
        IF( errstat /= 0 ) THEN
          WRITE( msg,'(A,2(A,I4),A)' ) "Calculate xnvalm failed at ",  &
...
!!!
      ...
      !Again use status rather than errstat as failure /= abort
      IF( status /= 0 ) THEN
        errflgs = 7
        QAflags = QAflags + errflgs
        if (use_he5_out) &
      ...
      if (use_he5_out) &
           CALL O3T_L2setDataPix( iT, iX, nwl_com, algflg, QAflags, &
           radBadPixflgs, stp1oz, stp2oz, stp3oz, oz_cld, aerind, so2ind,&
      ...
      if (use_tio_out) then
        ...
        if (errstat /= 0) then
          call tell_error(tell_io_error, &
               "O3T_main: l2_tio_write_fields failed", errstat)
          stop 1
        endif
      endif
    ...
    if (use_tio_out .and. have_omi_data) then
      ...
      if (errstat /= 0) then
        call tell_error(tell_io_error, &
             "O3T_main: l2_tio_write_mqf failed", errstat)
        stop 1
      endif
    endif
  ...
  if (use_tio_out) then
    ...
    if (errstat /= 0) then
      call tell_error(tell_io_error, &
           "O3T_main: l2_tio_close failed", errstat)
      stop 1
    endif
  endif

  !! free the allocated memory
  CALL O3T_freeIRR (errstat)
  CALL O3T_freeRAD (errstat)
    if (errstat /= 0) then
      call tell_error(tell_malloc_error, &
           "O3T_main: failed to free radiance / irradiance arrays", errstat)
      stop 1
    endif
  if (use_tio_in) then
    ...
    if (errstat /= 0) then
      call tell_error(tell_io_error, &
           "O3T_main: cld_close failed", errstat)
      stop 1
    endif
  else
    CALL O3T_freeCLD
  ...
  IF( errstat /= 0 ) THEN
    WRITE( msg,'(A)' ) "O3T_freeL2out failed, aborting."
    call tell_error (tell_runtime_error, msg, errstat)
    stop 1
  ENDIF

  !! free coefficient denominator data block
  errstat = O3T_lpolycden_dispose( LUT_cden_blk )
  IF( errstat /= 0 ) THEN
    WRITE( msg,'(A)' ) "O3T_lpolycden_dispose failed, aborting."
    call tell_error (tell_runtime_error, msg, errstat)
    stop 1
  ENDIF
  !! free the dndx tables.
  errstat = O3T_dndx_dispose()
  IF( errstat /= 0 ) THEN
    WRITE( msg,'(A)' ) "O3T_dndx_dispose failed, aborting."
    call tell_error (tell_runtime_error, msg, errstat)
    stop 1
  ENDIF

  !! clean up space allocated for NVAL LUT
  errstat = O3T_nval_dispose()
  IF( errstat /= 0 ) THEN
    WRITE( msg,'(A)' ) "O3T_nval_dispose failed, aborting."
    call tell_error (tell_runtime_error, msg, errstat)
    stop 1
  ENDIF
  ...
  if (have_omi_data) then
    DEALLOCATE(anomflg_3, stat=errstat)
    if (errstat /= 0) then
      call tell_error(tell_malloc_error, &
           "O3T_main: deallocate anomflg_3 failed", errstat)
      stop 1
    endif
  endif

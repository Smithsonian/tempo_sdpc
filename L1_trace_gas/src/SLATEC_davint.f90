MODULE SLATEC_davint

  use tell_module

  implicit none

  public dpolft, davint
  private dp1vlu
...
CONTAINS
  ...
  SUBROUTINE DAVINT (X, Y, N, XLO, XUP, ANS, IERR)
    ...
    INTEGER I, IERR, INLFT, INRT, ISTART, ISTOP, N, errstat
    ...
    call tell_error (tell_invalid_parm, &
         "davint: less than two function values were supplied", errstat)
    !     ...............EXIT
    GO TO 190
10  CONTINUE
    DO I = 2, N
...
40  CONTINUE
    ...
    call tell_error (tell_runtime_error, &
         "davint: <3 function values between the limits of integration", &
         errstat)
    !     ...............EXIT
    GO TO 190
50  CONTINUE
...
160 CONTINUE
    IERR = 2
    call tell_error (tell_runtime_error, &
         "davint: upper limit of integration is <= than lower limit", &
         errstat)
170 CONTINUE
    !     ......EXIT
    GO TO 190
180 CONTINUE
    IERR = 4
    call tell_error (tell_runtime_error, &
         "davint: abcissas not strictly increasing, must have x(i-1).lt.x(i) for all i", &
         errstat)
190 CONTINUE
    RETURN
...
!    INTEGER               :: &
!         nunit, nmsg, kflag, kountx, iunit, kunit, i, nerr, level, icount
...
!  END SUBROUTINE XGETUA
  ...
  SUBROUTINE DPOLFT (N, X, Y, W, MAXDEG, NDEG, EPS, R, IERR, A)
    ...
    INTEGER N, I,IDEGF,IERR,J,JP1,JPAS,K1,K1PJ,K2,K2PJ,K3,K3PI,K4,  &
         K4PI,K5,K5PI,KSIG,M,MAXDEG,MOP1,NDEG,NDER,NFAIL, errstat
    ...
    yp=0.0d0;sigpas=0.0d0;w1=0.0
    nfail=0;jpas=0;ksig=0
    M = ABS(N)
    IF (M .EQ. 0) GO TO 30
...
!         1)
    call tell_error(tell_invalid_parm, "dpolft: invalid input parameter", &
         errstat)
   GO TO 37
31  IERR = 3
    NDEG = MAXDEG
...
37  RETURN
  ...
  SUBROUTINE DP1VLU (L, NDER, X, YFIT, YP, A)
    ...
    INTEGER I,IC,ILO,IN,INP1,IUP,K1,K1I,K2,K3,K3P1,K3PN,K4,K4P1,K4PN,  &
         KC,L,LM1,LP1,MAXORD,N,NDER,NDO,NDP1,NORD, errstat
...
10  YFIT = VAL
    RETURN
    !
11  call tell_error (tell_runtime_error, &
         "dp1vlu: polynomial evaluation order exceeds highest order fit", &
         errstat)
    !WRITE (XERN1, '(I8)') L
    !WRITE (XERN2, '(I8)') NORD
    ...
    RETURN
    !
12  call tell_error (tell_invalid_parm, &
         "dp1vlu: input requests evaluation of negative order polynomial", &
         errstat)
    !CALL XERMSG ('SLATEC', 'DP1VLU',  &

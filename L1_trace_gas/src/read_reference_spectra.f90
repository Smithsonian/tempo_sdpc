...
SUBROUTINE read_ref_spectra ( pge_idx, n_max_rspec, pge_error_status )
  ...
  USE OMSAO_indices_module,    ONLY: &
    max_rs_idx, pge_static_input_luns, &
  ...
  DO i = 1, max_rs_idx
    IF (0 /= INDEX (TRIM(ADJUSTL(refspecs_original(i)%FileName)), &
                    zerospec_string)) then
      refspecs_original(i)%temperature = r8_missval
      cycle
    endif
  ...
  if (pge_idx == pge_o3_idx) then
    ...
    i = o3_t1_idx
    do while (associated (ptr))
      ptr%spectemp = refspecs_original(i)%temperature
      ptr => ptr % next
      i = i+1
    enddo

    ! Note that we are assigning the temperature to the uncertainty.  The
    ! original code did that, so do it here --JED  (FIXME, maybe)
    ptr => o3_prefit_uncert_he5fields % head
    i = o3_t1_idx
    do while (associated (ptr))
      ptr%spectemp = refspecs_original(i)%temperature
...
SUBROUTINE skip_headerlines ( funit, hstr, read_or_skip, nhead, lastline, errstat )
  ...
  CHARACTER (LEN=MAX_STR_LEN), INTENT (OUT)   :: lastline

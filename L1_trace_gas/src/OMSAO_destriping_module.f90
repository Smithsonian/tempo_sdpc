...
CONTAINS
  ...
  SUBROUTINE xtrack_destriping (ntimes, nxtrack, do_process_line, xtrange, &
    ...
    !REAL    (KIND=r8), DIMENSION (nxtrack,0:ntimes-1), INTENT (IN) :: saoamf
...
!UNUSED!     ! line, we can easily set the return values without
!UNUSED!     ! checking in detail. Actually, this should be true for
!UNUSED!     ! more than just 1 line difference, but for 2 or more
!UNUSED!     ! the loops further down can handle all cases.
!UNUSED!     ! -----------------------------------------------------
!UNUSED!     IF ( fline >= lline-1 ) THEN
!UNUSED!       latitude_lines(1) = fline
!UNUSED!       latitude_lines(2) = lline
!UNUSED!       avg_limits(1)     = fline
...
!UNUSED!     ! --------------------------------------------
!UNUSED!     upper_line = -1 ; lat1 = lat(nxh,nth)
!UNUSED!     LatUpper: DO j = nth, lline-1 !nth+1, ntimes-2
!UNUSED!       IF ( ALL(lat(nxh,j:j+1) >= lat1) ) THEN
!UNUSED!         upper_line = j+1 ; lat1 = lat(nxh,j+1)
!UNUSED!       ELSE
...
!UNUSED!         lower_line = j-1 ; lat1 = lat(nxh,j-1)
!UNUSED!       ELSE
...
!UNUSED!     ! --------------------------------------------------------
!UNUSED!     IF ( nblocks < 0 ) THEN
!UNUSED!       GetAvgLines: DO j = nth+1, ntimes-2
!UNUSED!         k = ntimes-1 - j
!UNUSED!         IF ( ALL(lat(nxh,k-1:k) < MINVAL(ctrdst_latrange)) .AND. &
!UNUSED!           avg_limits(1) == -1 ) avg_limits(1) = k + 1
!UNUSED!         IF ( ALL(lat(nxh,j:j+1) > MAXVAL(ctrdst_latrange)) .AND. &
!UNUSED!           avg_limits(2) == -1 ) avg_limits(2) = j - 1
...
!UNUSED!   END SUBROUTINE xtrack_destriping_lat_limits
  ...
  SUBROUTINE xtrack_pattern_polyfit ( &
      nxtrack, npol, xtrack_avg, xtrack_pos, xtrack_wgt, &

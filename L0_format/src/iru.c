                            ...
                            const IOCSDPC_IRU_Type *iru)
{
   ...
   if ((0 != TIO_label_product (pmt->ncid, TEMPO_PROD_TYPE_IRU, 0, pmt->processing_version))
                            ...
                            const IOCSDPC_Common_Header_Type *chdr)
{
   ...
   if (0 != make_level0_archdir_path (&pmt->archdir_path, timestamp, -1, TEMPO_PROD_TYPE_IRU))
     return -1;

...
contains
  ...
  SUBROUTINE radiance_fit ( fitcol, rms, dfitcol, exval )
    ...
    USE OMSAO_variables_module, ONLY: n_fincol_idx, &
         fincol_idx, pm_one, database, refspec_norm, renorm, weight_rad, &
         yn_doas, yn_smooth, rad_wav_avg, fitvar_rad, fitvar_rad_saved, &
         n_fitvar_rad, chisq, n_rad_wvl, lo_radbnd, up_radbnd, fitweights, &
         currspec, fitwavs, rad_winwav_idx, mask_fitvar_rad, max_itnum_rad, &
         curr_rad_spec!, sza_atm, vza_atm, sol_zen_eff, n_mol_fit, &
         !fitvar_rad_str, fitvar_rad_init, fitcol_idx
    use m_subtract_cubic, only: subtract_cubic_meas
    use m_specfit, only: specfit
    use fitting_functions, only: specfit_func
    ...
    ! Local variables
    ! ===============
    INTEGER  :: i, idx, n_fitwav_rad, j1, j2, k1, k2, l, ll_rad, lu_rad!, j
    REAL (KIND=dp) :: asum, ssum, remult, sigsum
    REAL (KIND=dp), DIMENSION (n_rad_wvl) :: fitres, fitspec, tmp, spec, &
         sig, pos
    REAL (KIND=dp), DIMENSION (n_max_fitpars) :: fitvar, lobnd, upbnd
    REAL (KIND=dp), DIMENSION (n_fitvar_rad, n_fitvar_rad) :: covar_matrix
    real (kind=dp), dimension (n_fitvar_rad) :: stderr
    ...
    ! ============================================================
    ll_rad = rad_winwav_idx(2)  
    lu_rad = rad_winwav_idx(3)
    ...
    IF ( yn_doas ) THEN
      spec(1:n_rad_wvl) = &
           LOG ( spec(1:n_rad_wvl) / database(solar_idx,1:n_rad_wvl) )
      CALL subtract_cubic_meas (pos, n_rad_wvl, spec, ll_rad, lu_rad)
      spec(1:n_rad_wvl) = &
           spec(1:n_rad_wvl) + LOG ( database(solar_idx,1:n_rad_wvl) )
    END IF
    ...
    CALL specfit ( &
         ...
         fitspec(1:n_rad_wvl), fitres(1:n_rad_wvl), stderr, exval, &
         specfit_func )
    ...
    CASE ( 1: )
      ...
      ! ----------------------------------------------------------------------
      ! Assign total column. We have done the preliminary work with 
      ! FITCOL_IDX and PM_ONE, so we don't have to "IF DOAS" here.
      !
      ! Reminder about the (admittedly confusing) variable names:
      !
      !   FITCOL:       Fitted column
      !   DFITCOL:      Uncertainty of fitted column
      !   N_FINCOL_IDX: Number of fitting variables that make up the final 
      !                  FITCOL (e.g., O3 at more than one temperature).
      !   FINCOL_IDX:   Array of dimension (2,N_FINCOL_IDX*MXS_IDX), 
      !                 where MXS_IDX is the maximum fitting sub-index 
      !                 (e.g., AD1_IDX, LBE_IDX). FINCOL_IDX(1,*) carries 
      !                 the relative indices of the varied
      ...
      DO i = 1, n_fincol_idx
        ...
        dfitcol = &
             dfitcol + covar_matrix(j1,j1)/(refspec_norm(k1)*refspec_norm(k1))
        !WRITE (www_lun,'(a,2I3,1p3E15.5)') '(a)', i, j1, fitvar(j1), covar_matrix(j1,j1), refspec_norm(k1)
        ...
        DO l = i+1, n_fincol_idx
          ...
          dfitcol = dfitcol + 2.0_dp * covar_matrix(j2,l) / &
               (refspec_norm(k1)*refspec_norm(k2))
        END DO
      END DO

      dfitcol = rms * SQRT ( dfitcol *  &
           REAL ( n_rad_wvl, KIND=dp ) / &
           REAL ( n_fitwav_rad - n_fitvar_rad, KIND=dp ) )

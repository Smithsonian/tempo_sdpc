...
contains
  ...
  SUBROUTINE lidort_prof_env (do_ozwf, do_albwf, do_tmpwf, do_o3shi, ozvary, &
    ...
    USE OMSAO_parameters_module,ONLY  : maxlay, max_fit_pts, du2mol, &
         rearth, max_spec_pts
    USE OMSAO_variables_module, ONLY  : rad_wav_avg, vza_atm, aza_atm, &
         radwvl_sav, n_radwvl_sav, fitvar_rad, rmask_fitvar_rad, &
         fitvar_rad_init, numwin, nradpix, nradpix_sav, avgsza, avgvza, &
         avgaza, b1ab_div_wav, database, database_shiwf, database_save, &
         refidx, band_selectors, refwvl, n_refwvl, refspec_norm, &
    ...
    USE ozprof_data_module,     ONLY  : nt_fit, atmosprof, nup2p, nflay, &
         ...
         gassidxs, fgassidxs, strat_aerosol, osfind, oswins, mflay, nfsfc, &
         nsfc, do_simu, maxgksec, ngksec, radcalwrt, wrtozcrs, &
    ...
    use lidort_prof_utilities, only: getabs_crs, get_alb_ozcrs_ray, &
         get_efft, get_hres_gascrs_ray, getso2_crs, get_tracegas_wf, &
         hres_radwf_inter_convol, lup_polerror, polcorr_online, &
         radwf_interpol
    ...
    ! Local variables
    ! =======================
    LOGICAL :: problems, ex, do_clouds, do_fozwf, do_faerwf, do_fraywf
    INTEGER :: status_inputread, status_inputcheck, status_calculation, &
         ic, iw, i, j, k, ii, kk, jj, jk, idum, low, hgh, fidx, lidx, nvza, &
         naza, nz1, tempi, nfgas, nstep, istk, npolmod, ipol, nsprs, npts, nw0
    ...
    REAL (KIND=dp), DIMENSION(nflay)             :: cldsca, cldext, cldasy, &
         eta, cldext0, aersca, aerext, aerasy
    ...
    IF (first) THEN
      ...
      CALL VLIDORT_L_INPUT_MASTER ( TRIM(tabdir)//'INP/vlidort_control.inp', &
    ...
    IF ( (polcorr >= 3 .AND. polcorr <= 5 ) .AND. nw > 1) THEN
      ...
      CALL polcorr_online(num_iter, polcorr, nw0, nz1, nctp, ncbp, nsprs, faer_lvl,        &
           ...
           prad(1:nw0), delabs(1:nw0, 1:nz1), ozs(1:nz1), albwf(1:nw0, 1), palbwf(1:nw0), &

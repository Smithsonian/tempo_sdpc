!
module m_amf_conversion

  public amf_conversion
  private esza2amf

contains

  SUBROUTINE amf_conversion ( n_arr_pix, sza, los, sol_zen_eff, amf, &
       amfgeo, have_amf )

    USE OMSAO_precision_module
    USE OMSAO_parameters_module, ONLY: deg2rad, rad2deg
    USE OMSAO_variables_module,  ONLY: szamax, amf_esza_min, amf_esza_max, &
         have_amftable
    ...
    IF ( ALL (ABS(sza(1:n_arr_pix)) <= szamax) .AND. &  
                                ! Solar zenith angles within bounds
         ALL (ABS(los(1:n_arr_pix)) <= szamax)       ) THEN  
      ! LineOfSight zenith angles within bounds

      ! ----------------------------------------------------------------------
      ! Compute the geometric AMF as the average over all geometric AMFs 
      ! for that pixel
      ! ----------------------------------------------------------------------
      ...
      ! -----------------------------------------------------------------------
      ! Table look-up of AMF if ESZA is within table bounds, 
      ! otherwise AMF=AMFGEO
      ! -----------------------------------------------------------------------
 ...
 have_amf = .FALSE.
  ...
  FUNCTION esza2amf ( sol_zen_eff )  RESULT ( amf )
    ...
    ! -------------------------------------------------------------------------
    ! Function to convert an Effective Solar Zenith Angle to an Air Mass Factor
    ! -------------------------------------------------------------------------
    ...
    ! ---------------
    INTEGER :: errstat
    real (kind=dp), dimension(1) :: temp_sol_zen_eff, temp_amf
    ...
    temp_sol_zen_eff = sol_zen_eff
    CALL interpolation ( n_amftab_ang, &
         amf_table(1:n_amftab_ang,amftab_ang_idx), &
         amf_table(1:n_amftab_ang,amftab_amf_idx), &
         1, temp_sol_zen_eff, temp_amf, errstat )
    amf = temp_amf(1)
    ! -------------------------------------------------------------------------
    ! Set AMF to Zero if ERRSTAT is not 0, i.e., if the interpolation has 
    ! encountered an error.
    ! -------------------------------------------------------------------------
    IF ( errstat /= pge_errstat_ok .AND. &
         verb_thresh_lev >= vb_lev_omidebug ) THEN
      errstat = OMI_SMF_setmsg (omsao_w_interpol, modulename, '', 0) 
      amf = 0.0_dp
    END IF

...
module lapack_tools
  ...
  public dgbtf2, dlabad, dlacon, dlassq, dlaswp, dlatbs, drscl, &
       dgetf2, dlatrs, ilaenv, lsame, ieeeck
...
contains
  ...
  SUBROUTINE DGBTF2( M, N, KL, KU, AB, LDAB, IPIV, INFO )
    use lapack_blas_tools, only: dger, dscal, dswap, idamax
    use m_xerbla
    !
  ...
  SUBROUTINE DLACON( N, V, X, ISGN, EST, KASE )
    use lapack_blas_tools, only: dcopy, idamax, dasum
    !
    !  -- LAPACK auxiliary routine (version 3.1) --
    !     Univ. of Tennessee, Univ. of California Berkeley and NAG Ltd..
    !     November 2006
...
150 CONTINUE
  ...
  SUBROUTINE DLATBS( UPLO, TRANS, DIAG, NORMIN, N, KD, AB, LDAB, X, &
    ...
    use lapack_blas_tools, only: daxpy, dscal, dtbsv, ddot, dasum, idamax
    use m_xerbla
    !
    !  -- LAPACK auxiliary routine (version 3.1) --
    !     Univ. of Tennessee, Univ. of California Berkeley and NAG Ltd..
    ...
    !EXTERNAL           LSAME, IDAMAX, DASUM, DDOT
    !     ..
    !     ..
    !     .. Intrinsic Functions ..
...
80    CONTINUE
    ...
    ELSE
      ...
      IF( NOTRAN ) THEN
        ...
        DO J = JFIRST, JLAST, JINC
          !
          !      ompute x(j) = b(j) / A(j,j), scaling x if necessary.
          !
          XJ = ABS( X( J ) )
          IF( NOUNIT ) THEN
...
150         CONTINUE
  ...
  SUBROUTINE DRSCL( N, SA, SX, INCX )
    use lapack_dlamch, only: dlamch
    use lapack_blas_tools, only: dscal
    !
    !  -- LAPACK auxiliary routine (version 3.1) --
    !     Univ. of Tennessee, Univ. of California Berkeley and NAG Ltd..
    !     November 2006
...
10  CONTINUE
  ...
  SUBROUTINE DLATRS( UPLO, TRANS, DIAG, NORMIN, N, A, LDA, X, SCALE, &
       CNORM, INFO )
    use lapack_dlamch, only: dlamch
    use lapack_blas_tools, only: daxpy, dscal, dtrsv, ddot, dasum, idamax
    use m_xerbla
    !
    !  -- LAPACK auxiliary routine (version 3.1) --
    !     Univ. of Tennessee, Univ. of California Berkeley and NAG Ltd..
    ...
    ELSE IF( SNAME .AND. C2.EQ.'OR' ) THEN
      IF( C3( 1: 1 ).EQ.'G' ) THEN
        IF( C4.EQ.'QR' .OR. C4.EQ.'RQ' .OR. C4.EQ.'LQ' .OR. C4.EQ. &
             'QL' .OR. C4.EQ.'HR' .OR. C4.EQ.'TR' &
             .OR. C4.EQ.'BR' ) THEN
          NB = 32
        END IF
      ELSE IF( C3( 1: 1 ).EQ.'M' ) THEN
        IF( C4.EQ.'QR' .OR. C4.EQ.'RQ' .OR. C4.EQ.'LQ' .OR. C4.EQ. &
             'QL' .OR. C4.EQ.'HR' .OR. C4.EQ.'TR' .OR. &
             C4.EQ.'BR' ) THEN
          NB = 32
        END IF
      END IF

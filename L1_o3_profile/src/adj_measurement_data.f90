...
contains
  ...
  SUBROUTINE adj_earthshine_data ( error )
    ...
    USE OMSAO_parameters_module,   ONLY : deg2rad, normweight!, rad2deg, pi, &
         !downweight
    USE OMSAO_variables_module,    ONLY : &
         sza_atm, vza_atm, amfgeo, szamax, &
         curr_rad_spec, amf, have_amf, sol_zen_eff, n_rad_wvl, &
         the_sza_atm, the_vza_atm, the_aza_atm, use_meas_sig, &
         the_month, the_day, the_lon, the_lat, &
         numwin, nradpix, the_surfalt, actspec_rad, database, &
         refidx, radwavcal_freq, yn_varyslit, slit_rad, &
         wavcal_sol, radnhtrunc, refnhextra, &
         nradpix_sav, n_radwvl_sav, radwvl_sav, tabdir
         !aza_atm, nsolpix, refwvl, the_sca_atm, the_year, zatmos, 
    USE OMSAO_gome_data_module,    ONLY : &
         n_gome_ang, n_gome_radpts, &
         gome_radspec, &
    ...
    USE ozprof_data_module,        ONLY : div_rad, div_sun, ozprof_flag, &
         ...
         do_lambcld, lambcld_refl, calunit!, glintprob, has_glint
    !! Commented by Kai
    ...
    use m_radiance_wavcal, only: radiance_wavcal
    ...
    ! Local variables
    ! ===============
    INTEGER     :: i, j, fidx, lidx, nhtrunc, ntrunc, &
         ntrunc1, idxoff!, ipix, k, isub
    !REAL (KIND=dp), DIMENSION (n_gome_ang)  :: zen_s, zen0_n, rel_azm
    ...
    IF (do_simu .AND. .NOT. radcalwrt) THEN
      OPEN(UNIT=l1l2inp_unit, FILE=TRIM(tabdir)//'INP/sim.inp', &
           STATUS='unknown')
      READ(l1l2inp_unit, *) the_sza_atm, the_vza_atm, the_aza_atm, &
           the_fixalb, the_surfalt, the_cfrac, the_ctp, the_cod, the_lon, &
           the_lat, the_month, the_day, which_aerosol, scaled_aod, &
           do_lambcld, lambcld_refl
    ...
    DO i = 1, numwin
      ...
      curr_rad_spec(1:sig_idx, fidx:lidx) = &
           curr_rad_spec(1:sig_idx, fidx + nhtrunc : lidx + nhtrunc)
...
!!$  ENDIF
    ...
    IF ( (gome_curpix == 1) .OR. MOD(gome_curpix, radwavcal_freq) == 0 ) THEN 
      ...
      IF (yn_varyslit) THEN 
        ...
        IF (.NOT. slit_rad .OR. wavcal_sol) CALL rad_wavcal_vary (calunit, &
             n_gome_radpts, curr_rad_spec(wvl_idx:sig_idx,1:n_gome_radpts), &
             error )
      ELSE 
    ...
    IF ( (gome_curpix == 1) .OR. &
!         MOD (gome_curpix, radwavcal_freq) == 0 .OR. redo_database) THEN 
! redo_database commented out above, so should not be used here
        MOD (gome_curpix, radwavcal_freq) == 0) THEN 
      ! --------------------------------------------------------------
      ! Spline data bases, compute undersampling spectrum, and prepare
      ! reference spectra for fitting.
      ! --------------------------------------------------------------
      CALL prepare_databases ( n_rad_wvl, &
           curr_rad_spec(wvl_idx,1:n_rad_wvl), pge_error_status )
...
!!$  ENDDO
    ...
    ! -------------------------------------------------------------------------
    ! Compute shape factor AMF from look-up tables. If the effective 
...
!
!    USE OMSAO_precision_module
!    USE OMSAO_indices_module,    ONLY: spc_idx, sig_idx!, wvl_idx
!    USE OMSAO_parameters_module, ONLY: maxwin!, downweight
!    USE OMSAO_variables_module,  ONLY: numwin, nradpix, band_selectors, &
!         tabdir!, &
!         !the_month, the_year, the_day, n_band_avg, n_band_samp, do_bandavg
...
!        IF (first) THEN
!          OPEN(UNIT=corr_unit, FILE=TRIM(tabdir)//'INP/b1a1b_merr_fudge_factor.dat', STATUS='OLD')
...
!    ENDIF
!
!    relsig = SQRT( relsig ** 2.0 + (solspec(sig_idx,:) / &
!         solspec(spc_idx,:)) ** 2.0)
!
!    IF (use_flns) THEN
!      fidx = 1
...
!    !WRITE(www_lun, '(3i5,2f8.2)') iwin, fidx, lidx, winlim(iwin,1), winlim(iwin, 2)
!
...
!         aza_atm, amfgeo, the_sza_atm, the_vza_atm, the_aza_atm, &
!         the_sca_atm!, szamax, sol_zen_eff
!    USE OMSAO_gome_data_module, ONLY: n_gome_ang, &
!         azm0_idx, azm_idx, zen0_idx, zen_idx, earth_curv, ers2_alt, &
!         gome_angles_wrtn, gome_angles_wrts, instrument_idx, &
!         scia_idx, gome2_idx!, omi_idx,  n_gome_geo, los_idx, gome_idx
!
!    IMPLICIT NONE
!
!    ! ===============
...
!      CALL scia_angle_sat2toa (earth_curv, ers2_alt, zatmos, n_gome_ang, &
!           zen0_n, zen_s, rel_azm, the_sza_atm, the_vza_atm, the_aza_atm, &
!           the_sca_atm )
!    ELSE IF (instrument_idx == gome2_idx ) THEN
!      CALL gome2_angle_sat2toa (earth_curv, ers2_alt, zatmos, n_gome_ang, &
!           zen0_n, zen_s, rel_azm, the_sza_atm, the_vza_atm, the_aza_atm, &
!           the_sca_atm )
!    ELSE 
!      CALL angle_sat2toa (earth_curv, ers2_alt, zatmos, n_gome_ang, zen0_n, &
!           zen_s, rel_azm, the_sza_atm, the_vza_atm, the_aza_atm, the_sca_atm )
!    ENDIF
!
!
!    ! ------------------------------
...
!    ! ------------------------------
!    zen0_n = rad2deg*zen0_n
!    zen_s = rad2deg*zen_s
!    rel_azm = rad2deg*rel_azm 
!    vza_atm = zen_s  
!    sza_atm = zen0_n
!    aza_atm = rel_azm
!
!    !WRITE(www_lun, '(A5, 3f8.2)')  'SZA =', zen0_n 
!    !WRITE(www_lun, '(A5, 3f8.2)')  'VZA =', zen_s 
!    !WRITE(www_lun, '(A5, 3f8.2)')  'AZA =', rel_azm
!    !WRITE(www_lun, '(A, 4f8.2)') 'Effective SZA, VZA, AZA, SCA = ', &
...
end module adj_measurement_data

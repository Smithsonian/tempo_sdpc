MODULE m_bounded_nonlin_LS
  ...
  !use LAPACK_BLAS_tools, only: dswap

  public elsunc, covar
  private lsunc, soliuc, newunc, scaunc, triunc, gndunc, btrunc, jacdif, &
       pivec, analuc, gnavuc, subuc, dimsub, pregn, presub, secuc, rtrw1, &
       jtrj, hess, termuc, stepuc, evreuc, reavuc, outuc, saveuc, grad, &
       releps, lineuc, linuc2, reduc, gauc, chder, quamin, minrn, update, &
       minrm, minrm1, minrm2, scalv, oner, twor, choose, fsumsq, sposl, &
       strsl, schdc, sqrdc, sqrsl, pol3, pol4, dnrm2
...
50  EXIT=error
  ...
  SUBROUTINE soliuc(x,n,f,m,ffunc,tau,scale,mdc,bl,bu,aset,nract,c,funcev,  &
    ...
    REAL (dp)  :: work(n), w1(m), w2(m), work2(n,1), tempw1(m)
    ...
    REAL (dp) :: cnorm,tol,scaqr(1),dykk(1),d1max,d1new,ymax
    real (dp) :: dummy1(1), dummy2(1), dummy3(1), dummy4(1)
    ...
    kmax=0
    w1(1:m)=-f(1:m)
    CALL sqrsl(c,m,n,qraux,w1,dummy1,tempw1,w2,dummy2,dummy3,1000,info)
    w1=tempw1
    ...
    DO  i=1,n
       ...
       ELSE
          ...
          IF(k /= kk) THEN
             ...
             CALL sqrsl(work2,mr,1,scaqr,w2(kk:),dummy1,w2(kk:),  &
                  dykk,dummy2,dummy3,100,info)
...
100    ip(1)=kk+1
       w2(ip(1):m)=zero
       IF(kmax /= kk) CALL sqrsl(work2,mr,1,scaqr,w2(kk:),w2(kk:),dummy1,  &
            dummy2,dummy3,dummy4,10000,info)
       work(kk:kk+mr-1) = work2(1:mr,1)

       !     V:= Q*H*(D1)
       !             ( 0)

       CALL sqrsl(c,m,n,qraux,w2,v,dummy1,dummy2,dummy3,dummy4,10000,info)
...
30  prank=k
  ...
  SUBROUTINE gndunc(f,m,c,n,scale,diag,p,qraux,prank,dx,d1sqs,v,work,y)
    ...
    INTEGER   :: i,info
    REAL (dp) :: temp, tempy(size(y))
    real (dp) :: dummy1(1), dummy2(1)
    ...
    CALL sqrsl(c,m,prank,qraux,y,dummy1,tempy,dx,dummy2,v,1101,info)
    y=tempy
...
30  secind=kod
  ...
  SUBROUTINE dimsub(restar,kod,fsum,f,m,c,n,  &
    ...
    REAL (dp) :: bn,sn,pgress,work(n),w1(m), tempw1(m)
    real (dp) :: dummy1(1), dummy2(1), dummy3(1), dummy4(1)
    ...
    IF ( .NOT. restar ) THEN 
      ...
      w1(1:m)=-f(1:m)
      CALL sqrsl(c,m,prank,qraux,w1,dummy1,tempw1,dummy2,dummy3,dummy4,1000,&
           info)
      w1=tempw1
...
10  dim=rngkm1
  ...
  SUBROUTINE secuc(ffunc,x,dx,diag,p,qraux,n,scale,f,v,c,mdc,m,mdg,nract,  &
    ...
    REAL (dp) :: dummy1(n), dummy2(1), dummy3(1), dummy4(1)
    ...
    nn=n - nract
    CALL sqrsl(c,m,nn,qraux,f,dummy1,w1,dummy2,dummy3,dummy4,1000,info)
...
240 RETURN
  ...
  ! Note private function of same name in LAPACK_ev.f90, but that version
  ! does not specify return value fn_val.
  !
...
! NOW PROVIDED BY LAPACK_BLAS_tools
!
! ! ============= dswap.f ==============
...
END MODULE m_bounded_nonlin_LS

...
module omi_cross_calibrate
  ...
  public omi_irrad_cross_calibrate, omi_rad_cross_calibrate, solwavcal_coadd, &
       radwavcal_coadd, prespec_align, prespec_align1, stray_coadd
  private
...
contains
  ...
  SUBROUTINE omi_irrad_cross_calibrate (first_pix, last_pix, pge_error_status)
    ...
    USE OMSAO_variables_module, ONLY: curr_sol_spec, n_irrad_wvl, &
         ...
         sswav_sol, nslit_sol, nwavcal_sol, numwin, solslitfit, solwavfit, &
         currpix, currpixchar, scnwrt, wavcal, reduce_resolution, &
         band_selectors!, slitwav, slitfit, slitdis, nslit
    ...
    DO ix = first_pix, last_pix
      ...
      !! Kai
      IF (scnwrt) THEN
        OPEN(UNIT=999,FILE="Before_IRR_IX"//currpixchar//".txt", FORM="FORMATTED" )
      ...
      !! Kai
      IF (scnwrt) THEN
        OPEN(UNIT=999,FILE="After_IRR_IX"//currpixchar//".txt", FORM="FORMATTED" )
        WRITE(999,'(F12.5,2G16.8)') (curr_sol_spec(wvl_idx,i), &
             curr_sol_spec(spc_idx,i), &
             curr_sol_spec(sig_idx,i), i=1,n_irrad_wvl)
        CLOSE(999)
  ...
  SUBROUTINE omi_rad_cross_calibrate (first_pix, last_pix, pge_error_status)
    ...
    USE OMSAO_variables_module,  ONLY: curr_rad_spec, n_rad_wvl, &
         ...
         radwavfit, radwinfit, nslit, slitwav, slitfit, nwavcal_rad, &
         sswav_rad, currpix, currpixchar, which_slit, scnwrt, wavcal!, slitdis
    USE OMSAO_omidata_module,    ONLY: omi_radiance_spec, &
         ...
         omi_radwavfit, omi_sswav_rad, nxbin, nybin, offset_line!, nfxtrack, &
         !omi_radiance_errstat, omi_nwav_irrad, omi_irradiance_wavl, 
    USE ozprof_data_module, ONLY: calunit
    USE OMSAO_errstat_module
    ...
    DO ix = first_pix, last_pix
      ...
      !! Kai
      IF (scnwrt) THEN
        OPEN(UNIT=999,FILE="Before_RAD_IX"//currpixchar//".txt", FORM="FORMATTED" )
      ...
      !! Kai
      IF (scnwrt) THEN
        OPEN(UNIT=999,FILE="After_RAD_IX"//currpixchar//".txt", FORM="FORMATTED" )
        WRITE(999,'(F12.5,2G16.8)') (curr_rad_spec(wvl_idx,i), &
             curr_rad_spec(spc_idx,i), &
             curr_rad_spec(sig_idx,i), i=1,n_rad_wvl)
        CLOSE(999)

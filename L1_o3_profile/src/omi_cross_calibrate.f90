!
module omi_cross_calibrate

  public omi_irrad_cross_calibrate, omi_rad_cross_calibrate, solwavcal_coadd, &
       radwavcal_coadd, prespec_align, stray_coadd!, prespec_align1
  private
...
contains
  ...
  SUBROUTINE omi_irrad_cross_calibrate (first_pix, last_pix, pge_error_status)
    ...
    USE OMSAO_indices_module, ONLY: wvl_idx, spc_idx, sig_idx, &
         max_calfit_idx, hwe_idx!, vgr_idx, shi_idx
    USE OMSAO_parameters_module, ONLY: normweight!, downweight
    USE OMSAO_variables_module, ONLY: curr_sol_spec, n_irrad_wvl, &
         use_meas_sig, yn_varyslit, which_slit, wavcal_sol, nsolpix, &
    ...
    ! ------------------------------
    CHARACTER (LEN=25), PARAMETER :: modulename = 'omi_irrad_cross_calibrate'
    ...
    DO ix = first_pix, last_pix
      ...
      curr_sol_spec(wvl_idx,1:n_irrad_wvl) = &
           omi_irradiance_wavl(1:n_irrad_wvl, ix)
      curr_sol_spec(spc_idx,1:n_irrad_wvl) = &
           omi_irradiance_spec(1:n_irrad_wvl, ix)

      IF (use_meas_sig) THEN
        curr_sol_spec(sig_idx, 1:n_irrad_wvl) = &
             omi_irradiance_prec(1:n_irrad_wvl, ix)
      ELSE
      ...
      !! Kai
      IF (scnwrt) THEN
        OPEN(UNIT=999,FILE="Before_IRR_IX"//currpixchar//".txt", &
             FORM="FORMATTED" )
        WRITE(999,'(F12.5,2G16.8)') (curr_sol_spec(wvl_idx,i), &
             curr_sol_spec(spc_idx,i), &
             curr_sol_spec(sig_idx,i), i=1,n_irrad_wvl)
        ! WRITE(999, *) omi_solnorm(ix)
        CLOSE(999)
      ENDIF
      !! Kai
      ...
      sol_spec_ring(1, 1:nsol_ring) = &
           omi_solspec_ring(1, solring_lin(ix):solring_uin(ix), ix)

      ! Perform calibration
      IF (scnwrt) WRITE(*, '(A,I5)') &
           'Performing solar wavelength calibration: ', (ix - 1) * nxbin + 1

      IF (scnwrt) WRITE(*, *) 'yn_varyslit=',yn_varyslit, &
      ...
      IF (yn_varyslit) THEN
        IF (which_slit < 5 ) THEN
          ...
          omi_solslitfit(1:nslit_sol, 1:max_calfit_idx, :, ix) = &
        ...
        IF (wavcal .AND. (wavcal_sol .OR. which_slit == 5)) THEN
          ...
          !omi_solwavfit(1:nwavcal_sol, 1:max_calfit_idx, :, ix) = &
          !    solwavfit(1:nwavcal_sol, 1:max_calfit_idx, :)
      ...
      ELSE
        IF (wavcal .OR. which_slit /= 5) THEN
          ...
          ENDIF
          omi_solwinfit(1:numwin, 1:max_calfit_idx, :, ix) = &
               solwinfit(1:numwin, 1:max_calfit_idx, :)
        ENDIF
      END IF
      !! Kai
      IF (scnwrt) THEN
        OPEN(UNIT=999,FILE="After_IRR_IX"//currpixchar//".txt", &
             FORM="FORMATTED" )
        WRITE(999,'(F12.5,2G16.8)') (curr_sol_spec(wvl_idx,i), &
             curr_sol_spec(spc_idx,i), &
             curr_sol_spec(sig_idx,i), i=1,n_irrad_wvl)
    ...
    !DO ix = first_pix, last_pix
    !   WRITE(90, '(I5, 2D16.7)') ix, omi_solwinfit(1, hwe_idx, 1, ix),  &
    !     omi_solwinfit(1, shi_idx, 1, ix)
    !ENDDO
  ...
  SUBROUTINE omi_rad_cross_calibrate (first_pix, last_pix, pge_error_status)
    ...
    use m_rad_fit_vary
    use m_rad_wavcal_vary
    use m_radiance_wavcal
    ...
    DO ix = first_pix, last_pix
      ...
      IF (ALL( omi_radpix_errstat(ix, 0:ntimes_loop-1) == pge_errstat_error)) &
           CYCLE

      ! Only process a scan line in the middle
      DO i = mline, 0, -1  ! first half
        IF (omi_radpix_errstat(ix, i) == pge_errstat_ok .AND. &
             omi_nwav_rad(ix, i) == np) EXIT
      ENDDO
      ...
      curr_rad_spec(wvl_idx, 1:n_rad_wvl) = &
           omi_radiance_wavl(1:n_rad_wvl, ix, iline)
      curr_rad_spec(spc_idx, 1:n_rad_wvl) = &
           omi_radiance_spec(1:n_rad_wvl, ix, iline)

      IF (use_meas_sig) THEN
        curr_rad_spec(sig_idx, 1:n_rad_wvl) = &
             omi_radiance_prec(1:n_rad_wvl, ix, iline)
      ELSE
      ...
      !! Kai
      IF (scnwrt) THEN
        OPEN(UNIT=999,FILE="Before_RAD_IX"//currpixchar//".txt", &
             FORM="FORMATTED" )
        WRITE(999,'(F12.5,2G16.8)') (curr_rad_spec(wvl_idx,i), &
             curr_rad_spec(spc_idx,i), &
             curr_rad_spec(sig_idx,i), i=1,n_rad_wvl)
        ! WRITE(999, *) omi_solnorm(ix)
        CLOSE(999)
      ENDIF
      !! Kai
      IF (which_slit < 5) THEN
        IF (yn_varyslit .AND. slit_rad ) THEN
          ...
          omi_radslitfit(1:nslit_rad, 1:max_calfit_idx, :, ix) = &
      ...
      IF (wavcal) THEN
        IF (yn_varyslit .AND. (wavcal_sol .OR. which_slit == 5) ) THEN
          ...
          !omi_radwavfit(1:nwavcal_rad, 1:max_calfit_idx, :, ix) = &
          !     radwavfit(1:nwavcal_rad, 1:max_calfit_idx, :)
          omi_sswav_rad(1:nwavcal_rad, ix) = sswav_rad(1:nwavcal_rad)
        ELSE
          CALL radiance_wavcal (n_rad_wvl, &
               curr_rad_spec(wvl_idx:sig_idx, 1:n_rad_wvl), error)
          omi_wincal_wav(1:numwin, ix) = wincal_wav(1:numwin)
          omi_radwinfit(1:numwin, 1:max_calfit_idx, :, ix) = &
               radwinfit(1:numwin, 1:max_calfit_idx, :)
        END IF
      ENDIF
      ...
      !! Kai
      IF (scnwrt) THEN
        OPEN(UNIT=999,FILE="After_RAD_IX"//currpixchar//".txt", &
             FORM="FORMATTED" )
        WRITE(999,'(F12.5,2G16.8)') (curr_rad_spec(wvl_idx,i), &
             curr_rad_spec(spc_idx,i), &
             curr_rad_spec(sig_idx,i), i=1,n_rad_wvl)
      ...
      DO i = 0, ntimes_loop - 1
        IF (omi_radpix_errstat(ix, i) == pge_errstat_ok) THEN
          ...
          IF (np == n_rad_wvl) THEN
            omi_radiance_wavl(1:np, ix, i) = &
                 real(omi_radiance_wavl(1:np, ix, i) + diff(1:np) , kind=r4)
            IF ( ANY(omi_radiance_wavl(2:np, ix, i) - &
                 omi_radiance_wavl(1:np-1, ix, i) < 0.0) ) THEN
              omi_radiance_wavl(1:np, ix, i) = &
                   real(omi_radiance_wavl(1:np, ix, i) - diff(1:np) , kind=r4)
          ...
          ELSE  ! Perform interpolation
            CALL INTERPOL(wav(1:n_rad_wvl), diff(1:n_rad_wvl), n_rad_wvl,  &
                 real(omi_radiance_wavl(1:np, ix, i), kind=8), corr(1:np), &
                 np, inter_errstat)
            ...
            ELSE
              omi_radiance_wavl(1:np, ix, i) = &
                   real(omi_radiance_wavl(1:np, ix, i) + corr(1:np) , kind=r4)
              IF ( ANY(omi_radiance_wavl(2:np, ix, i) - &
                   omi_radiance_wavl(1:np-1, ix, i) < 0.0) ) THEN
                omi_radiance_wavl(1:np, ix, i) = &
                     real(omi_radiance_wavl(1:np, ix, i) - corr(1:np), kind=r4)
  ...
  SUBROUTINE solwavcal_coadd(wcal_bef_coadd, nspec, ncoadd, allspec, wshis, &
       wsqus, error)
  ...
  SUBROUTINE radwavcal_coadd(wcal_bef_coadd, wavcal, & ! iwin, ix, &
    ...
    IF (wcal_bef_coadd .AND. wavcal) THEN
      ...
      !      fitvar_sol(hwe_idx:hwr_idx) = &
      !            omi_solwinfit(iwin, hwe_idx:hwr_idx, 1, ix)
      ...
      DO i = 1, ncoadd
        ...
        IF (solfit_exval < 0) THEN
          WRITE(www_lun, *) &
               'radwavcal_coadd: calibration not converge for pixel: ', i
          error = .TRUE.
          wshis(i) = 0.0; wsqus(i) = 0.0
        END IF
        ...
        ! Make sure that the correction is less than a pixel
        dwvl = fitwavs(2) - fitwavs(1)
        IF ( ANY( ABS(allspec(i, wvl_idx, 1:nspec) - &
             fitwavs(1:nspec)) > dwvl ) ) THEN
          allspec(i, wvl_idx, 1:nspec) = fitwavs(1:nspec) ! Roll back
          wshis(i) = 0.0; wsqus(i) = 0.0
        ELSE
          wshis(i) = fitvar_sol(shi_idx); wsqus(i) = fitvar_sol(squ_idx)
...
end module omi_cross_calibrate

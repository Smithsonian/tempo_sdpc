!> LAPACK routines used in o3 profile
module gsvd_o3prof_utilities

  !use lapack_tools, only: lsame, ilaenv
  !use LAPACK_BLAS_tools
  !use LAPACK_drivers
  !use LAPACK_dlamch, only: dlamch
  !use LAPACK_ev, only: dlartg, dorg2r, dlacpy, dlaset, dlarf, dlarfg, &
  !     dtrmm, dtrmv, dnrm2
  !use m_xerbla

  public dgetri, dtrtri, dtrti2, dggsvd, choldc, dsvdcmp, fmin, dpythag

  private dorm2r, dormr2, dtgsja, dgeqpf, dlapll, dlags2, dlas2, &
       dlasv2, dlapmt, dgeqr2, dgerq2, dggsvp, dpythag1!, &
       !dgauss, invert, dsvdcmp1
    ...
    DO  KCYCLE = 1, MAXIT
      ...
      DO  I = 1, L - 1
        DO  J = I + 1, L
          ...
          A2 = ZERO
          A3 = ZERO
          IF( K+I.LE.M ) A1 = A( K+I, N-L+I )
          IF( K+J.LE.M ) A3 = A( K+J, N-L+J )
          !
          B1 = B( I, N-L+I )
          B3 = B( J, N-L+J )
          !
...
80      CONTINUE
  ...
  SUBROUTINE DGGSVP( JOBU, JOBV, JOBQ, M, P, N, A, LDA, B, LDB, &
    ...
    !      WRITE(www_lun, *) 'Before call DGEQPF'
    CALL DGEQPF( P, N, B, LDB, IWORK, TAU, WORK, INFO )
    !      WRITE(www_lun, *) 'After call DGEQPF'
    !
    !     Update A := A*P
    !
    !      WRITE(www_lun, *) 'Before call DLAPMT'
    ...
    !      WRITE(www_lun, *) 'After call WANTU'
    !
    ...
    IF( WANTU ) THEN
      ...
      IF( M.GT.1 ) &
           CALL DLACPY( 'Lower', M-1, N-L, A( 2, 1 ), LDA, U( 2, 1 ), LDU ) 
...
!    !     hw1e (half-width at 1/e intensity). Assumes input spectrum has
!    !     constant spacing.
!
...
190 fmin=x
  ...
  !     Routine to perform Cholesky decomposition
  !     From Numerical Recipes by W.H. Press [1992]
...
!  END subroutine dsvdcmp1
  ...
  FUNCTION dpythag1(a,b)
    DOUBLE PRECISION a,b,dpythag1
    DOUBLE PRECISION absa,absb
    absa=abs(a)
    absb=abs(b)
    if(absa.gt.absb)then
      dpythag1=absa*sqrt(1.0d0+(absb/absa)**2)
    else
      if(absb.eq.0.0d0)then
        dpythag1=0.0d0
      else
        dpythag1=absb*sqrt(1.0d0+(absa/absb)**2)
      endif
    endif
    return
  END function dpythag1
...
end module gsvd_o3prof_utilities

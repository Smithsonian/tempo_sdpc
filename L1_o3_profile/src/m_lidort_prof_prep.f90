!
module m_lidort_prof_prep

  public lidort_prof_prep, get_all_raycof, get_all_raycof_depol, &
       get_all_raycof_depol1
  private  !prepare_spherical
...
contains
  ...
  SUBROUTINE LIDORT_PROF_PREP (raycof, depol, zsgrid, airgrid,  varyprof, &
    ...
    integer :: i, j, k, q, nscatter, idx, cldidx, aeridx, &
         nactgksec, nactgkmatc!, ui
    INTEGER, DIMENSION(allngas) :: absin
    REAL (KIND=dp) :: scaco_r, absco_r, extco_r, extco, scaco, pvar, &
         extco_a, scaco_a, extco_c, scaco_c, j0, j1
...
!  END SUBROUTINE PREPARE_SPHERICAL
  ...
  SUBROUTINE GET_ALL_RAYCOF_DEPOL1(nw, waves, nw1, raycof, depol, problems)
    ...
    USE OMSAO_variables_module,  ONLY : n_refspec_pts, &
         refspec_orig_data, do_bandavg!, refdbdir
    USE OMSAO_errstat_module

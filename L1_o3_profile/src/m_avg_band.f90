...
contains
  ...
  SUBROUTINE avg_band_radspec(currspec, npts, errstat)
    ...
    INTEGER :: i, iwin, k, npix, npts_out, fidx, lidx, navg, nsamp, ntemp!, j

    errstat = pge_errstat_ok
    IF (npts /= SUM(nradpix(1:numwin))) THEN
      WRITE(www_lun, *) 'avg_band_radspec: Number of points inconsistent!!!'
  ...
  SUBROUTINE avg_band_refspec(pos, spec, npts_in, npts_out, errstat)
    ...
    INTEGER  :: i, iwin, k, npix, fidx, lidx, navg, nsamp, nextra!, j
    ...
    ELSE
      WRITE(www_lun, *) 'avg_band_refspec: Number of points inconsistent!!!'
      WRITE(www_lun, *) npts_in, n_radwvl_sav, SUM(nradpix(1:numwin))
      errstat = pge_errstat_ok; RETURN
    ENDIF
...
!      npts_out = npts_in;      RETURN
!    ELSE
!      WRITE(www_lun, *) 'avg_band_effrefspec: Number of points inconsistent!!!'
!      WRITE(www_lun, *) npts_in, n_radwvl_sav, SUM(nradpix(1:numwin))
!      errstat = pge_errstat_ok; RETURN
!    ENDIF
!
...
!  END SUBROUTINE avg_band_effrefspec
  ...
  SUBROUTINE avg_band_effozcrs(pos, spec, npts_in, npts_out, errstat)
    ...
    REAL (KIND=dp)                     :: sumsol, sumf
    INTEGER, DIMENSION(numwin)         :: winpts
    INTEGER  :: i, iwin, k, npix, fidx, lidx, navg, nsamp!, j
    ...
    ELSE IF (npts_in == SUM(nradpix(1:numwin))) THEN
      npts_out = npts_in;      RETURN
    ELSE
      WRITE(www_lun, *) 'avg_band_effozcrs: Number of points inconsistent!!!'
      WRITE(www_lun, *) npts_in, n_radwvl_sav, SUM(nradpix(1:numwin))
      errstat = pge_errstat_ok; RETURN
    ENDIF
  ...
  SUBROUTINE avg_band_ozcrs(pos, spec, npts_in, npts_out, errstat)
    ...
    ELSE IF (npts_in == SUM(nradpix(1:numwin))) THEN
      npts_out = npts_in;      RETURN
    ELSE
      WRITE(www_lun, *) 'avg_band_ozcrs: Number of points inconsistent!!!'
      WRITE(www_lun, *) npts_in, n_radwvl_sav, SUM(nradpix(1:numwin))
      errstat = pge_errstat_ok; RETURN
    ENDIF
...
!
!    USE OMSAO_precision_module
!    USE OMSAO_variables_module, ONLY: numwin, n_band_avg, &
!         n_band_samp, dwavmax!, nradpix_sav, nradpix, n_radwvl_sav
!    USE OMSAO_errstat_module
!
!    IMPLICIT NONE
!
...
!    REAL (KIND=dp)                     :: sumsol
!    INTEGER, DIMENSION(numwin)         :: winpts
!    INTEGER  :: i, k, npix, fidx, lidx, navg, nsamp!, j
!
!    spec_temp = 0.0 !initalize spectra to be returned
!
...
!  END SUBROUTINE avg_band_subrefspec
  ...
  SUBROUTINE avg_band_spec(pos, spec, npts_in, npts_out, errstat)
    ...
    IF (npts_in /= SUM(nradpix_sav(1:numwin))) THEN
      WRITE(www_lun, *) 'avg_band_spec: Number of points inconsistent!!!'
      WRITE(www_lun, *) npts_in, SUM(nradpix_sav(1:numwin))
      errstat = pge_errstat_ok; RETURN
    ENDIF

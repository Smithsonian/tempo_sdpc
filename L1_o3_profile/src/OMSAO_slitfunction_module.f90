...
CONTAINS
  ...
  SUBROUTINE load_slitpars (pge_error_status)
    ...
    CHARACTER(LEN=3), DIMENSION(NCH), PARAMETER :: sltables = &
         (/'uv1', 'uv2', 'vis'/)
    ...
    DO ic = 1, NCH
      ...
      OPEN (UNIT=calunit, FILE=TRIM(ADJUSTL(slitpar_fname)), &
           STATUS='UNKNOWN', IOSTAT=errstat)
      IF ( errstat /= pge_errstat_ok ) THEN
        WRITE(www_lun, '(2A)') modulename, &
             ': Cannot open slit parameterization file!!!'
      ...
      IF (nx /= nxtrack_max .AND. nx /= nxtrack_max / 2) THEN
        WRITE(www_lun, '(2A)') modulename, &
             ': # of across track positions does not match!!!'
        pge_error_status = pge_errstat_error
 RETURN
      ENDIF
    ...
    IF (coadd_uv2) THEN
      DO ix = 1, nfxtrack
        ...
        DO j = 1, ncoadd-1      
          omi_slitpars(2:3, ix, :, :) = omi_slitpars(2:3, ix, :, :) + &
               omi_slitpars(2:3, i + j, :, :)
        ENDDO
    ...
    IF (nxbin > 1) THEN
      DO ix = 1, nfxtrack / nxbin
        ...
        DO j = 1, nxbin - 1      
          omi_slitpars(:, ix, :, :) = omi_slitpars(:, ix, :, :) + &
               omi_slitpars(:, i + j, :, :)
        ENDDO
  ...
  SUBROUTINE compute_slitprofile (ch, lamda, wave, nw, slit)
    ...
    ENDIF
    !WRITE(www_lun, '(6D16.8)') pars
    !WRITE(www_lun, '(15D12.4)') slit(1:nw) / MAXVAL(slit)

    RETURN

  END SUBROUTINE compute_slitprofile

  ! Use an average slit function for each fitting window instead of 
  ! using variable slit widths
  SUBROUTINE omislit_multi (wvlarr, specarr, specmod, npoints)
    ...
    DO iw = 1, numwin
      IF (iw < numwin) THEN
        ...
        lidx = MINVAL(MAXLOC(wvlarr(fidx:npoints), &
             MASK=(wvlarr(fidx:npoints) < bndwav))) + fidx - 1
      ELSE
        lidx = npoints
      ENDIF
      ...
      IF (ch == 1) THEN
        nhalf = INT(1.0 / delwvl, kind=4)  ! 2 nm for UV-1 (truncate these < ~0.001)
      ELSE 
        nhalf = INT(0.5 / delwvl, kind=4)  ! 1 nm for UV-2 and VIS (truncate those < ~0.001)
      ENDIF
      nslit = nhalf * 2 + 1
      ...
      CALL compute_slitprofile (ch, wvlarr(i), wvlarr(i-nhalf:i+nhalf), &
           nslit, locsli(1:nslit))
      slitsum = SUM(locsli(1:nslit))

      ! safe convolution mode 
      DO i = sidx, eidx
        specmod(i) = DOT_PRODUCT(locsli(1:nslit), &
             specarr(i-nhalf:i+nhalf)) / slitsum
      ENDDO
    ...
    DO iw = 1, numwin
      ...
      DO i = sidx, eidx
        CALL compute_slitprofile (ch, wvlarr(i), wvlarr(i-nhalf:i+nhalf), &
             nslit, locsli(1:nslit))
        specmod(i) = DOT_PRODUCT(locsli(1:nslit), &
             specarr(i-nhalf:i+nhalf)) / SUM(locsli(1:nslit))
      ENDDO
      ...
      DO i = fidx, sidx - 1
        ...
        lpnt = nhalf - i + 1
        idxs(1 : lpnt) = ABS(idxs(1 : lpnt)) + 2
        CALL compute_slitprofile (ch, wvlarr(i), wvlarr(idxs(1:nslit)), &
             nslit, locsli(1:nslit))
        specmod(i) = DOT_PRODUCT(locsli(1:nslit), &
             specarr(idxs(1:nslit))) / SUM(locsli(1:nslit))
      ENDDO
  ...
  !xliu, 10/29/2009, add the capability to convole multiple spectrum 
  !simultaneously
 ...
 lidxc = nc
      ...
      DO i = fidxc, lidxc
        ...
        CALL compute_slitprofile (ch, cwave(imid), tmpwav(sidx:eidx), &
             eidx-sidx+1, slit(sidx:eidx))
  ...
  SUBROUTINE omislit_vary_f2c (fwave, fspec, nf, nspec, cwave, cspec, nc)
    ...
    DO iwin = 1, numwin
      ...
      DO i = fidxc, lidxc
        ...
        ENDDO
        eidx = j
        CALL compute_slitprofile (ch, cwave(i), fwave(sidx:eidx), &
             eidx-sidx+1, slit(sidx:eidx))

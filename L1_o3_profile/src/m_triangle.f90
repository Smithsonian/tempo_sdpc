...
contains
 ...
 IF ( j2 < 1 ) j2 = ABS(j2) + 2
  ...
  SUBROUTINE triangle_vary (wvlarr, specarr, specmod, npoints)
    ...
    INTEGER  :: i, j, j1, j2, num_slit, mslit, sslit, &
         eslit, errstat,  fidx, lidx, fslit, lslit, finter, linter!, iwin, odd
    REAL (KIND=dp)                      :: delwvl, maxslit, fwhm!, slitsum
    REAL (KIND=dp), DIMENSION (npoints) :: slit, locwvl, locsli, lochwe!, xxx
    INTEGER,        DIMENSION (npoints) :: idx
 ...
 idx = (/ (i, i = 1, npoints) /)
    ...
    ! get an array of slit variables (lochwe, locasy) for each 
    ! wavelength position
    lochwe = 0.0
 ...
 fidxc = lidxc + 1
  ...
  SUBROUTINE triangle_vary_f2c (fwave, fspec, nf, nspec, cwave, cspec, nc)
    ...
    DO iwin = 1, numwin
      ...
      IF (lslit < fslit + 3) THEN  ! extrapolate, use the nearest value
        lochwe(fidxc:lidxc) = &
             SUM(slitfit(fslit:lslit, hwe_idx, 1))/(lslit-fslit+1)
      ELSE
        ! finter <= linter here

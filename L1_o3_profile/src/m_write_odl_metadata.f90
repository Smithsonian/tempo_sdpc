...
contains
  ...
  !-----------------------------------------------------------------------
  function write_odl_metadata(l1bfile, outfilnm, versionid, &
                              lun_input, ninp) result(errstat)

    use OMI_LUN_set, only : MCF_FILE_LUN
    implicit none

    !input variables
    character (len=*), intent(in) :: l1bfile, outfilnm
    integer (kind=4), intent(in) :: versionid
    ...
    call tiof_push_group (tio_l1obj, rad_swathname(1), errstat)
    ...
    returnstatus = pgs_met_init(MCF_FILE_LUN, GROUPS)
    if (returnstatus /= 0 ) then
      call tell_error(tell_io_error, &

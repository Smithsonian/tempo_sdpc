!
module m_get_reg_matrix

  public get_reg_matrix
  private
...
contains
  ...
  SUBROUTINE get_reg_matrix(sa, ptr_nump, ptr_b, pge_error_status)
    ...
    ELSE IF (ptr_order == 6) THEN      ! use a priori covariance matrix L^T * L = Sa^(-1) 
      ptr_nump = nlay_fit   
    ELSE
      WRITE(www_lun, *) modulename, ' : This order of PTR is not implemented!!!'
      pge_error_status = pge_errstat_error; RETURN
    END IF

    IF (ptr_nump <= 3) THEN
      WRITE(www_lun, *) modulename, ' : Please increase # regularized parameters!!!'
      pge_error_status = pge_errstat_error; RETURN
    END IF
    ...
    ELSE IF (ptr_order == 4) THEN
      ...
      IF ( pge_error_status == pge_errstat_error) THEN
        WRITE(www_lun, *) modulename, ' : CHOLDC failed!!!'
        RETURN
      END IF

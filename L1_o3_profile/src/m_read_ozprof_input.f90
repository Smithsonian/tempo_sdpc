...
contains
  ...
  SUBROUTINE read_ozprof_input (fit_ctrl_unit, fit_ctrl_file, &
       pge_error_status)
    ...
    USE ozprof_data_module, ONLY: nlay, nlay_fit, ozstr, albstr, othstr, &
         ...
         aerosol, strat_aerosol, use_lograd, polcorr, t_fidx, t_lidx, &
         ...
         nothgrp, use_reg_presgrid, presgrid_fname, use_tropopause, &
         adjust_trop_layer, pst0, fixed_ptrop, ntp0, which_biascorr, &
    ...
    USE OMSAO_indices_module, ONLY: max_rs_idx,  max_calfit_idx, mxs_idx, &
         ...
         !solar_idx, so2_idx, shi_idx, no2_t1_idx, n_max_fitpars, hcho_idx, &
         !bro_idx
    USE OMSAO_variables_module, ONLY: fitvar_rad_init, fitvar_rad_saved, &
         ...
         fothvarpos, fitvar_rad_unit!, refspec_fname
    ...
    IF (do_multi_vza) THEN
      WRITE(www_lun, *) &
           'Only effective viewing geometry is computed for a ground pixel!!!'
      pge_error_status = pge_errstat_error
      RETURN
    ENDIF
    ...
    IF (which_aperr > 5 .OR. which_aperr <= 0) THEN
    ...
    IF (which_toz > 2 .OR. which_toz < 0) THEN
      ...
      RETURN
    ENDIF
    IF (which_clima == 1 .AND. which_toz == 0) THEN
      WRITE(www_lun, *) modulename, &
           ' Total ozone is needed to use this climatology!!!'
      pge_error_status = pge_errstat_error
      RETURN
    ENDIF
    ...
    IF (nmom > maxmom) THEN
      WRITE(www_lun, *) modulename, &
           ': Input # of phase moments exceeds maxmom: ', maxmom
    ...
    IF (hres_samprate < 0.01) THEN
      WRITE(www_lun, *) modulename, ':hres_samprate must be > 0.01!!!'
      pge_error_status = pge_errstat_error
      RETURN
    ENDIF
    ...
    IF ( MOD(hres_samprate * 100, 1.0) /= 0) THEN
      WRITE(www_lun, *) modulename, &
           ': hres_samprate must be multiples of 0.01!!!'
      hres_samprate = NINT(hres_samprate * 100) / 100.
      WRITE(www_lun, *) modulename, &
           ': hres_samprate is reset to: ', hres_samprate
    ENDIF
    ...
    IF ( MINVAL(radc_samprate(1:radc_nsegsr)) < hres_samprate ) THEN
      WRITE(www_lun, *) modulename, ': radc_samprate must be > hres_samprate!!!'
      pge_error_status = pge_errstat_error
      RETURN
    ENDIF
    ...
    IF (nwfc > maxwfc) THEN
      ...
      RETURN
      !ELSE IF (nwfc < 1) THEN
      !   WRITE(www_lun, *) modulename, ' : Need to specify at least 1 cloud fraction term!!!'
      !   pge_error_status = pge_errstat_error; RETURN     
    ELSE IF (nwfc < 1) THEN
      nwfc = 0
    ENDIF
    ...
    DO i = 1, ncldaer
      ...
      IF ( i == 8 ) THEN ! For SO2Z, SO2V needs to be selected
        ...
        IF (rmask_fitvar_rad(j) == 0) THEN
          vartmp = 0.0
          lotmp = 0.0
          uptmp = 0.0          ! Disable SO2Z if SO2V is not selected
        ELSE                                               
          IF (vartmp == 0.0 .AND. lotmp >= uptmp) THEN 
            vartmp = 5.0
            lotmp = 5.0
            uptmp = 5.0       ! Initialize to 5 km if not initialized or set to zero
          ENDIF
        ENDIF
      ENDIF
    ...
    DO i = 1, nothgrp        ! for each group of parameters
      ...
      DO j = 1, ntotp
        IF (lo_radbnd(idx + j) < up_radbnd(idx + j)) THEN
          ...
          theord = NINT(1.0 * j / np)
          thewin = j - (theord - 1) * np

!
module m_solar_wavcal_vary

  public solar_wavcal_vary
  private
...
contains
  ...
  SUBROUTINE solar_wavcal_vary (swavcal_unit, error )
    ...
    REAL (KIND=dp), DIMENSION (n_slit_par, n_irrad_wvl)    :: tmpslit 
    REAL (KIND=dp), DIMENSION (n_irrad_wvl) :: allwaves, locshi, locsqu, &
         locspec
    INTEGER :: npoints, i, iwin, fidx, lidx, errstat = pge_errstat_ok, &
         iwavcal, fpos, lpos, fwavcal, lwavcal, ios, finter, linter, &
         npoly, solfit_exval, sfidx, slidx!, nstep, n_fit_pts, j
    ...
    IF (which_slit /= 5) THEN
      ...
      DO i = 1, n_slit_par
        IF (slitfit(1, slitind(i), 1) /= 0.0) &
             ...
             nslit, allwaves(fpos:lpos), tmpslit(i, fpos:lpos), &
             lpos-fpos+1, errstat)
        IF (errstat < 0) THEN
    ...
    ELSE
      ...
      DO 
        ...
        IF (ios < 0) THEN
          ...
          EXIT  ! end of file
        ELSE IF (ios > 0) THEN
          WRITE(www_lun, *) 'Solar_wavcal_vary: read slit file error'
          error = .TRUE.
          RETURN
        END IF
      END DO
    ...
    IF (smooth_slit) THEN
      ...
      DO iwin = 1, numwin
        ...
        IF (lwavcal > fwavcal + 3) THEN
          !CRN 30-Sep-2009:  use 4th order polynomial for all windows, 
          !not just first
          !IF (iwin == 1) THEN
          poly_order = 5
          !ELSE
          ...
          DO i = shi_idx, squ_idx
            IF (solwavfit(fwavcal,i,1)==0. .AND. &
                 solwavfit(lwavcal,i,1)==0.0) CYCLE
            ...
            solwavfit(fwavcal:lwavcal, i, 1) = &
                 solwavfit(fwavcal:lwavcal, i, 1) - locspec(1:npoly)           
          END DO
        ELSE
          DO i = shi_idx, squ_idx
            solwavfit(fwavcal:lwavcal, i, 1) = &
              SUM(solwavfit(fwavcal:lwavcal, i, 1)) / (lwavcal-lwavcal + 1.0)
          END DO
        END IF
        fidx = lidx + 1
      END DO
    ...
    DO i = 1, numwin
      ...
      ELSE IF (correct_lamda == 2) THEN 
       ...
       IF (linter < slidx) sol_spec_ring(1, linter+1:slidx) = &
          (sol_spec_ring(1, linter+1:slidx) - locshi(lidx) + sol_wav_avg * locsqu(lidx)) / (1.0 + locsqu(lidx))
      ENDIF
...
end module m_solar_wavcal_vary

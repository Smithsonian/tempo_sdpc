...
contains
  ...
  SUBROUTINE GET_CLOUD_MIPROP(waves, nwave, useasy, gcq, gcw, gcasy, ngksec, gcmoms, errstat) 
    ...
    IF (first) THEN
      allocate(phfcn(MWL, 0:maxmom, 1:maxgksec))
      ! --------------------------------------------------------------
      ...
      READ(atmos_unit, *) nwl
      IF (nwl > MWL) THEN
        WRITE(www_lun, *) modulename, ' # cloud input wavelengths exceeds MWL. Increase MWL!!!'
  ...
  SUBROUTINE GET_CLOUD_MAPROP (month, stpix, sndpix, cldflg, &
    ...
    LOGICAL, SAVE     :: first = .TRUE.
    INTEGER           :: i, nump, stat, apix, pixtyp, nactp, nactp1, actyr, &
         actmon!, j
    ...
    ELSE IF (cldflg == 5) THEN
      nump = 30
    ELSE
      WRITE(www_lun, *) 'GET_CLOUD_MAPROP: Should not happen!!!'
      errstat = pge_errstat_error; RETURN
    ENDIF
...
!    ! Local variables
!    ! ======================
!    INTEGER, PARAMETER :: nlat=72, nlon=144!, numy = 8
...
!      IF (.NOT. file_exist) THEN
!        WRITE(www_lun, *) 'ISCCP file not exist: ', isccp_fname
!        errstat = pge_errstat_error; RETURN
!      ENDIF
!
...
!  END SUBROUTINE GET_ISCCP_CTP
  ...
  ! =============================================================
  ! Obtain TOMSV8 monthly mean cloud-top pressure
  ! =============================================================
  SUBROUTINE GET_TOMSV8_CTP(month, day, lon, lat, ctp, errstat)
    ...
    REAL (KIND=dp), SAVE, DIMENSION(:,:,:), POINTER :: glbctp
    ...
    IF (first) THEN
      ...
      allocate(glbctp(nmon, nlon, nlat))
      ! determine lon and lat index
      ...
      INQUIRE (FILE= isccp_fname, EXIST= file_exist)
      IF (.NOT. file_exist) THEN
        WRITE(www_lun, *) 'OMCLDRR file not exist: ', isccp_fname
        errstat = pge_errstat_error; RETURN
      ENDIF

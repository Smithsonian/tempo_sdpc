!
module m_subtract_cubic

  public subtract_cubic, subtract_cubic_meas
  private
...
contains
  ...
  SUBROUTINE subtract_cubic ( locwvl, npts, ll_rad, lu_rad )
    ...
    allocate (f (nfitted), dfda(nfitted, doas_npol))
    ...
    DO i = 1, max_rs_idx
      IF ( i /= solar_idx ) THEN
        ...
        p(3) = 5  
        w = -1.0
        blow(1:doas_npol) = 0.0_dp  
        bupp(1:doas_npol) = 0.0_dp
        ...
        f = 0.0_dp 
        dfda = 0.0_dp
        CALL elsunc ( par, doas_npol, nfitted, nfitted, cubic_specfit, &
             elbnd, blow(1:doas_npol), bupp(1:doas_npol), p, w, exval, &
             f(1:nfitted), dfda(1:nfitted,1:doas_npol) )
        chisq = SUM  ( f(1:nfitted)**2 ) ! This gives the same CHI**2 as the NR routines
        x(1:npts) = locwvl(1:npts) - locavg

        cubic_x(1:npts) = x(1:npts)
        exval = 3
        CALL cubic_specfit ( par(1:doas_npol), doas_npol, cubic_y(1:npts), &
             npts, exval, dfda, 0 )
  ...
  SUBROUTINE subtract_cubic_meas ( locwvl, npoints, locspec, ll_rad, lu_rad )
    ...
    REAL (KIND=dp), DIMENSION (doas_npol)           :: r!, par, polfunc
   ...
   CALL elsunc ( r, doas_npol, nfitted, nfitted, cubic_specfit, elbnd, &
         blow(1:doas_npol), bupp(1:doas_npol), p, w, exval, f(1:nfitted), &
         dfda(1:nfitted,1:doas_npol) )
    ...
    exval = 3
    CALL cubic_specfit ( r(1:doas_npol), doas_npol, cubic_y(1:npoints), &

...
!    REAL (KIND=dp) :: delwvl, slitsum,  maxslit, voigt, left_norm,&
    REAL (KIND=dp) :: delwvl, slitsum,  maxslit, left_norm, &
    ...
    ! --------------------------------------------------------------
    left_norm =  voigt(0.0_r8, vgl, hwl)
    right_norm = voigt(0.0_r8, vgr, hwr)
    ...
    DO i = -npoints/2, npoints/2
      ...
      !WRITE(www_lun, *) locwvl(j), locsli(j)
    END DO
    ...
    !WRITE(www_lun, *) sslit, eslit, num_slit, mslit
    ...
    END DO
    !WRITE(www_lun, *) 'num_slit = ', num_slit
    !DO i = 1, num_slit
  ...
  SUBROUTINE asym_voigt_multi (wvlarr, specarr, specmod, npoints)
    ...
    ! Local variables
    ! ===============
    INTEGER        :: i, j, j1, j2, num_slit, mslit, sslit, eslit, &
         iwin, fidx, lidx!, odd
!    REAL (KIND=dp) :: delwvl, slitsum,  maxslit, voigt, upbnd, left_norm, &
    REAL (KIND=dp) :: delwvl, slitsum,  maxslit, upbnd, left_norm, &
         right_norm, norm, vg, hw1e, vgr, vgl, hwr, hwl
    REAL (KIND=dp), DIMENSION (npoints) :: slit, locwvl, locsli!, xxx
    INTEGER,        DIMENSION (npoints) :: idx
    ...
    DO iwin = 1, numwin
      ...
      ! ----------------------------------------------------------------------
      ! Find array entries that mark the region of SLIT >= 0.0001*MAX(SLIT).
      ! We start by initializing the auxilliary IDX as IDX(n) = n. We then set
      ! all those entries to 0 that are outside the accepted minimum value of 
      ! the slit function. The start and end indices simply follow as the 
      ! minimum and maximum values of IDX where IDX /= 0.
  ...
  SUBROUTINE asym_voigt_vary (wvlarr, specarr, specmod, npoints)
    ...
    ! Local variables
    ! ===============
    INTEGER        :: i, j, j1, j2, num_slit, mslit, errstat, fidx, lidx, &
         fslit, lslit, finter, linter!, iwin, eslit, odd, sslit
    REAL (KIND=dp) :: delwvl, hwr, hwl, hw1e, vg, vgr, vgl, left_norm, &
         right_norm!, voigt, norm, maxslit, slitsum
    REAL (KIND=dp), DIMENSION (npoints) :: slit, locwvl, locsli, &
         lochwl, lochwr, locvgl, locvgr!, xxx
    INTEGER,        DIMENSION (npoints) :: idx
  ...
  ! xliu, 10/29/2009, add the capability to convole multiple 
  ...
  SUBROUTINE asym_voigt_f2c (fwave, fspec, nf, nspec, cwave, cspec, nc)
    ...
    ! Local variables
    ! ===============
    INTEGER  :: i, j, iwin, fidx, fidxc, lidx, lidxc, midx, sidx, eidx!, iw
    REAL (KIND=dp) :: temp, hwl, hwr, vgr, vgl, lnorm, rnorm, &
         ssum!, voigt, hw, vg
    ...
    DO iwin = 1, numwin
      ...
      vgl = solwinfit(iwin, vgl_idx, 1)
      vgr = solwinfit(iwin, vgr_idx, 1)
      lnorm = voigt(0.0_r8, vgl, hwl)
      rnorm = voigt(0.0_r8, vgr, hwr)

      DO i = fidxc, lidxc
        ! Find the closest pixel
        midx = MINVAL(MAXLOC(fwave(fidx:lidx), &
             MASK=(fwave(fidx:lidx) <= cwave(i)))) + fidx
  ...
  SUBROUTINE asym_voigt_vary_f2c (fwave, fspec, nf, nspec, cwave, cspec, nc)
    ...
    USE OMSAO_precision_module
    USE OMSAO_variables_module, ONLY : winlim, numwin, slit_trunc_limit, &
    ...
    INTEGER        :: i, j, iwin, fidx, fidxc, lidx, lidxc, &
         midx, sidx, eidx, fslit,lslit, finter, linter, errstat!, iw
    REAL (KIND=dp) :: temp, hwl, hwr, vgr, vgl, lnorm, rnorm, &
         ssum!, voigt, hw, vg
    REAL (KIND=dp), DIMENSION (nf) :: slit
    ...
    ! Name of this subroutine/module
    ! ------------------------------
    CHARACTER (LEN=19), PARAMETER :: modulename = 'asym_voigt_vary_f2c'
    ...
    DO iwin = 1, numwin
      ...
      IF (lslit < fslit + 3) THEN  ! extrapolate, use the nearest value
        lochwl(fidxc:lidxc) = &
             SUM(slitfit(fslit:lslit, hwl_idx, 1))/(lslit-fslit+1)
        lochwr(fidxc:lidxc) = &
             SUM(slitfit(fslit:lslit, hwr_idx, 1))/(lslit-fslit+1)
        locvgl(fidxc:lidxc) = &
             SUM(slitfit(fslit:lslit, vgl_idx, 1))/(lslit-fslit+1)
        locvgr(fidxc:lidxc) = &
             SUM(slitfit(fslit:lslit, vgr_idx, 1))/(lslit-fslit+1)
      ELSE
        ! finter <= linter here
        ...
        CALL interpolation ( lslit-fslit+1, slitwav(fslit:lslit), &
             slitfit(fslit:lslit, hwl_idx, 1), linter-finter+1, &
             cwave(finter:linter), lochwl(finter:linter), errstat )
        IF ( errstat > pge_errstat_warning ) THEN
          errstat = OMI_SMF_setmsg (omsao_e_interpol, modulename, '', 0) 
          STOP 1
        END IF
        ...
        CALL interpolation ( lslit-fslit+1, slitwav(fslit:lslit), &
             slitfit(fslit:lslit, hwr_idx, 1), linter-finter+1, &
             cwave(finter:linter), lochwr(finter:linter), errstat )
        IF ( errstat > pge_errstat_warning ) THEN
          errstat = OMI_SMF_setmsg (omsao_e_interpol, modulename, '', 0) 
          STOP 1
        END IF
        ...
        CALL interpolation ( lslit-fslit+1, slitwav(fslit:lslit), &
             slitfit(fslit:lslit, vgl_idx, 1), linter-finter+1, &
             cwave(finter:linter), locvgl(finter:linter), errstat )
        IF ( errstat > pge_errstat_warning ) THEN
          errstat = OMI_SMF_setmsg (omsao_e_interpol, modulename, '', 0) 
          STOP 1
        END IF
        ...
        CALL interpolation ( lslit-fslit+1, slitwav(fslit:lslit), &
             slitfit(fslit:lslit, vgr_idx, 1), linter-finter+1, &
             cwave(finter:linter), locvgr(finter:linter), errstat )
        IF ( errstat > pge_errstat_warning ) THEN
          errstat = OMI_SMF_setmsg (omsao_e_interpol, modulename, '', 0) 
          STOP 1
        END IF

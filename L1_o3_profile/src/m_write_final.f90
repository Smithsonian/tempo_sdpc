!
module m_write_final

  public write_final, omi_write_intermed
...
!  END SUBROUTINE gome_write_intermed
  ...
  SUBROUTINE omi_write_intermed (founit, fitcol, dfitcol, rms, exval )
    ...
    USE ozprof_data_module,      ONLY: ozprof, ozprof_std, &
         ...
         weight_function, do_simu, glintprob!, ozprof_end_index, &
         !ozprof_start_index, which_alb, which_cld, &
         !use_oe, maxawin, ozprof_init
    USE OMSAO_variables_module, ONLY : the_sza_atm, the_vza_atm, &
         the_aza_atm, the_lons, the_lats, nloc, the_utc, fitvar_rad, &
    ...
    WRITE(founit, '(A11,2I5,1X,A28)') 'Line/XPix: ', &
         currline * nybin + offset_line + 1, (currpix -1) * nxbin + 1, the_utc
    DO i = 1, nloc
    ...
    IF (exval >= 0) THEN
      ...
      simspec_rad(1:n_rad_wvl) = &
           fitspec_rad(1:n_rad_wvl) - fitres_rad(1:n_rad_wvl)
      IF (use_lograd) THEN
        fitspec_rad(1:n_rad_wvl) = EXP(fitspec_rad(1:n_rad_wvl))
        simspec_rad(1:n_rad_wvl) = EXP(simspec_rad(1:n_rad_wvl))
        fitres_rad(1:n_rad_wvl) = &
             fitspec_rad(1:n_rad_wvl) -  simspec_rad(1:n_rad_wvl)
      END IF
      avgres = SQRT(SUM((ABS(fitres_rad(1:n_rad_wvl)) / &
           fitspec_rad(1:n_rad_wvl))**2.0)/n_rad_wvl)*100.0   
      ...
      WRITE(founit, '(14f8.3)') &
           rms, avgres, ozdfs, ozinfo, allrms(1:numwin), allavgres(1:numwin)

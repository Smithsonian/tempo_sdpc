!
module m_gsvd_lcurve_gcv

  public gsvd_lcurve_gcv, inverse
  private sort_gsvd, scurv, fcurv, find_lcorner, swap, get_gsvd_sol
...
contains
  ...
  SUBROUTINE gsvd_lcurve_gcv (ozwrtint, ozwrtint_unit, epsrel, num_iter, &
       n_rad_wvl, n_fitvar_rad, ptr_nump, ptr_b, g_spec, dyda, xap, xold, &
       xname, fopt, covar, conv, avg_kernel, contri, ozdfs, ozinfo, nchisq)
    ...
    ! Local variables
    ! ---------------
    INTEGER :: i, j, ptr_k, gsvd_info, inv_info!, k
    ...
    REAL (KIND=dp) :: lcurve_gcv_ratio, ptr_alpha1, &
         ptr_alpha2,  OptEng, OptRho, OptCurv, OptEng2, OptRho2, OptCurv2,&
         ochisq, delchi, gtemp, ptr_alpha!, temp, picard, gamsq
    ...
    !	print the results
    IF (gsvd_info /= 0) THEN
      WRITE(www_lun, *) modulename, ' : Call DGGSVD Fails !!!'
      pge_error_status = pge_errstat_error; GO TO 999
    END IF

    !	compute x, and sort the generalized singular values in increasing order
    CALL sort_gsvd (acpy, bcpy, u(1:n_rad_wvl, 1:n_rad_wvl), q, &
         alpha(1:n_fitvar_rad), beta(1:n_fitvar_rad), n_rad_wvl, &
         n_fitvar_rad, ptr_nump, ptr_k, ptr_l, iwork, &
         x(1:n_fitvar_rad, 1:n_fitvar_rad), gamma(1:n_fitvar_rad), &
         pge_error_status)
    ...
    CALL dgetrf(n_fitvar_rad, n_fitvar_rad, xinv, n_fitvar_rad, ipiv, &
         inv_info )
    IF (inv_info /= 0)  THEN
      WRITE(www_lun, *) modulename, ' :LU of x fails and inv_info = ', inv_info
      pge_error_status = pge_errstat_error; GO TO 999;
    ELSE
      CALL dgetri(n_fitvar_rad, xinv, n_fitvar_rad, ipiv, &
           real(work_inv, kind=8), n_fitvar_rad, inv_info)
      IF (inv_info /= 0)  THEN
        WRITE(www_lun, *) modulename, &
             ' :Inverse of x fails and inv_info = ', inv_info
        pge_error_status = pge_errstat_error; GO TO 999;
      END IF
    END IF
    ...
    ELSE IF (lcurve_gcv == 3) THEN  ! use gcv only unless gcv/lcurve differs by 10, use lcurve
      ...
      END IF
    ELSE
      WRITE(www_lun, *) modulename, ' : This option is not implemented for lcurve_gcv!!!'
      pge_error_status = pge_errstat_error; GO TO 999
    END IF

    IF (pge_error_status == pge_errstat_error) THEN
      WRITE(www_lun, *) modulename, ' :compute best regularization parameter'; GO TO 999
    END IF


    !WRITE(www_lun, *) 'ptr_alpha = ', ptr_alpha
...
999 WRITE(www_lun, *) modulename, 'Error occur in GSVD_LCURVE_GCV'
  ...
  SUBROUTINE sort_gsvd (a, b, u, q, alpha, beta, m, n, p, k, l, iwork, x, &
    ...
    IF (info /= 0) THEN
      WRITE(www_lun, *) modulename, ' : Call dtrtri Fails !!!'
...
! FIXME - not clear whether low beta should produce gamma=0 or very large
  ...
  SUBROUTINE scurv(rlamda, TheCurv, TheRho, TheEng)
    ...
    ! ---------------	
    INTEGER                     :: i  
    REAL (KIND=dp)              :: TheEngfd, picard, sumf, denom, &
         gamsq, lamsq!, picard1, picard2
  ...
  SUBROUTINE find_lcorner(ptr_alpha, OptRho, OptEng, OptCurv, pge_error_status)
    ...
    USE OMSAO_variables_module, ONLY  : lcurve_unit
    ...
    REAL (KIND=dp) :: CurvMin, gmax, gmin, DeltaLam, alam, blam!, & 
         !OptLam, fmin, fcurv
    ...
    IF (ptr_l > maxlam) THEN
      WRITE(www_lun, *) modulename, ' : Find_lcorner: Increase maxlam to at least ',  ptr_l
      pge_error_status = pge_errstat_error; RETURN
    END IF

...
MODULE L2_attr_class
    ...
    CHARACTER( LEN = 80 ):: inventoryMetadataName = "CoreMetadata"
    CHARACTER( LEN = 80 ):: archivedMetadataName  = "ArchiveMetadata"
...
!        CHARACTER (LEN=PGSd_PC_FILE_PATH_MAX) :: InputL2filename
         CHARACTER( LEN=PGSd_MET_MAX_STRING_SET_L) :: ShortName, &
                                                      InputPGEVersion,&
                                                      InputVersions, &
                                                      OrbitData

         CHARACTER( LEN = 40 ), DIMENSION(npcfattr) ::  &
         ...
         CHARACTER( LEN = 255 ) :: StringValue!, strTemp
         INTEGER (KIND=4) :: version 
         INTEGER (KIND=4) :: status, ierr, di
         INTEGER (KIND=4) :: SW_fileid!, SW_id, InputL2fileid
         ...
         DO di = 1, SIZE( inputLUNs )
           ...
           IF( status /= PGS_S_SUCCESS ) THEN
              status = PGS_MET_getPCAttr_s( inputLUNs(di), version , &
                                   TRIM(inventoryMetadataName)//'.0', &
                                   "SHORTNAME", ShortName )
              IF( status /= PGS_S_SUCCESS ) THEN
                 WRITE( msg,'(A,I8)' ) "get ShortName failed at LUN:", inputLUNs(di)
                 ierr = OMI_SMF_setmsg( OMI_E_INPUT, msg, &
                                    "OMI_writeGlobalAttribute", zero )
                 status = OMI_E_FAILURE
                 return
           ...
           IF( status /= PGS_S_SUCCESS ) THEN
              ...
              IF( status /= PGS_S_SUCCESS ) THEN
                 WRITE( msg,'(A,I8)' ) &
                             "get InputPGEVersion failed at LUN:", &
                                   inputLUNs(di) 
                 ierr = OMI_SMF_setmsg( OMI_E_INPUT, msg, &
                                    "OMI_writeGlobalAttribute", zero )
                 status = OMI_E_FAILURE
...
!!!      WRITE(*,*) 'InputVersions=',TRIM(InputVersions)
         ...
         DO di = 1, npcfattr
           ...
           IF( status /= PGS_S_SUCCESS ) THEN
              WRITE( msg,'(A,I8)' ) "get from PCF failed at LUN = ", lun(di)
              ierr = OMI_SMF_setmsg( status, msg, &
                                     "OMI_writeGlobalAttribute", &
                                     zero )
              status = OMI_E_FAILURE
              RETURN
           ENDIF
         ...
         IF( status /= PGS_S_SUCCESS ) THEN
            status = PGS_MET_getPCAttr_s( L1B_UV_FILE_LUN, version , &
                                   TRIM( archivedMetadataName)//'.0', &
                                      "ORBITDATA", OrbitData )
            IF( status /= PGS_S_SUCCESS ) THEN
                WRITE( msg,'(A,I8)' ) "get OrbitData failed at LUN:", L1B_UV_FILE_LUN
                ierr = OMI_SMF_setmsg( OMI_E_INPUT, msg, &
                                    "OMI_writeGlobalAttribute", zero )
                OrbitData = "Unknown"
            ENDIF
         ...
         nc = LEN_TRIM(OrbitData) 
         numtype = HE5T_NATIVE_CHAR
         ierr = he5_ehwrglatt( SW_fileid, "OrbitData", numtype, nc, &
                               TRIM(OrbitData) )
         IF( ierr == -1 ) THEN
            WRITE( msg,'(A)' ) "he5_ehwrglatt: OrbitData "// &
                               "failed in file " // TRIM(L2_filename )

...
CONTAINS
  ...
  ! Asymmetric factors and phase moments are computed for a range of reff and
  ...
  SUBROUTINE READ_AEROSOL_PROF(year, month, lon, lat, ps, zs, tp, nz, &
    ...
    ! Parameter Variables
    ! ======================
    INTEGER, PARAMETER :: NAER = 6, MWL=10, NWL=4, NSLAYER=31, NLAT=91, &
         NSLAT=28, NLON=144
    !INTEGER, PARAMETER :: NTLAYER = 26 (for using v4.26)
    INTEGER, PARAMETER :: NTLAYER = 21  !(for using GEOS-3 and 4 fields)
    REAL (KIND=dp), PARAMETER :: latgrid=2.0, latsgrid=5.0, longrid=2.5, &
         lon0=-181.25, lat0=-91.0
    ...
    REAL (KIND=dp)                  :: xg, divalt, slope, frac!, tau_in, xg1
    ...
    REAL (KIND=DP), DIMENSION(0:NTLAYER), PARAMETER:: pres3 = (/ &
         1.00000D+00, 9.97095D-01, 9.91200D-01, 9.81500D-01, 9.67100D-01, &
         9.46800D-01, 9.19500D-01, 8.84000D-01, 8.39000D-01, 7.83000D-01, &
         7.18200D-01, 6.47600D-01, 5.74100D-01, 5.00000D-01, 4.27800D-01, &
         3.59500D-01, 2.97050D-01, 2.41950D-01, &
    ...
    REAL (KIND=DP), DIMENSION(0:NTLAYER), PARAMETER:: ap4    = (/  &
         ...
         127.837006D0, 108.663429D0, &
         92.365662D0,  78.512299D0,  56.387939D0, 40.175419D0/)
    REAL (KIND=DP), DIMENSION(0:NTLAYER), PARAMETER:: bp4    = (/  &
         1.000000D0,   0.985110D0,   0.943290D0,   0.867830D0, 0.764920D0,  &
         0.642710D0,   0.510460D0,   0.378440D0,   0.270330D0, 0.183300D0, &
         0.115030D0,  0.063720D0,  0.028010D0,   0.006960D0,   0.000000D0, &
    ...
    IF (first) THEN
      ! saved variables, just allocating here, not deallocating later
      ...
      allocate (phfcn(maxgksec, 0:maxmom, MWL, NAER))
      !---------------------------------------------------------------
      ...
      IF (.NOT. file_exist) THEN
        WRITE(www_lun, *) &
          'Warning: no aerosol profile file found, use 96-97, 99-00 average!!!'
      ...
      READ(atmos_unit, '(144E8.2)') &
           ((((tarsl(i,j,k,n),i=1,NLON),j=1,NLAT),k=1,NTLAYER),n=1,NAER)
      CLOSE(atmos_unit)
    ...
    IF (strat_aerosol) THEN
      ...
      DO ib = 1, nblat
        ...
        nsmoms(1:NWL, 1:NSLAYER, 0:nmom, 1:ngksec) =  &
             nsmoms(1:NWL, 1:NSLAYER, 0:nmom, 1:ngksec) &
             + smoms(1:NWL, latin(ib), 1:NSLAYER, 0:nmom, 1:ngksec) * &
             latfrac(ib)
      ENDDO
    ...
    DO i = 1, nwave
      DO j = faer, laer
        ...
        ELSE
          phsmoms(1:ngksec, 0:nmom, j) = &
               phfcn(1:ngksec, 0:nmom, low, j)*(1.0 - xg) + &

...
subroutine GEMSTOOL_VLIDORT_Initialize &
   ( GEMSTOOL_INPUTS, nlayers, VLIDORT_FixIn, VLIDORT_ModIn, VLIDORT_Sup, fail, message )
...
!  Input from the GEOPHYS setup (Proxy here)
...
!   if ( GEMSTOOL_INPUTS%RTMControl%do_firstorder_option ) then
...
!   endif
   ...
   VLIDORT_FixIn%Bool%TS_DO_UPWELLING  = GEMSTOOL_INPUTS%RTMControl%do_Upwelling
...
!   if ( GEMSTOOL_INPUTS%RTMControl%do_SphericalAlbedo ) then
...
!   else
      if ( GEMSTOOL_INPUTS%RTMControl%do_regular_ps ) then
         VLIDORT_ModIn%MBool%TS_DO_SSCORR_OUTGOING    = .FALSE.
...
!  FURTHER UPGRADE, 03 July 2018. No longer using the internal FO code inside VLIDORT
!     -- FO Code is now External.
...
!   VLIDORT_ModIn%MBool%TS_DO_FO_CALC = .false. 
   VLIDORT_ModIn%MBool%TS_DO_FO_CALC = GEMSTOOL_INPUTS%RTMControl%do_firstorder_option 
...
!      Performance enhancements need to used with care......... default, turn them off
   ...
   IF ( .not.VLIDORT_ModIn%MBool%TS_DO_RAYLEIGH_ONLY ) THEN
      VLIDORT_ModIn%MBool%TS_DO_DOUBLE_CONVTEST = .false. ! jbak
      VLIDORT_ModIn%MBool%TS_DO_DELTAM_SCALING  = .false.
      VLIDORT_ModIn%MBool%TS_DO_SOLUTION_SAVING = .false.
      VLIDORT_ModIn%MBool%TS_DO_BVP_TELESCOPING = .false.
   ENDIF
...
!    -- Number of discrete ordinates from GEMSTOOL RT control configuration input
   ...
   VLIDORT_FixIn%Cont%TS_NSTOKES          = GEMSTOOL_INPUTS%RTMControl%NVlidort_nstokes
   VLIDORT_FixIn%Cont%TS_NSTREAMS         = GEMSTOOL_INPUTS%RTMControl%NVlidort_nstreams
...
!    -- Number of layers determined from PTH profile, but input here
...
!   VLIDORT_FixIn%Cont%TS_NFINELAYERS      = 4 
...
!    -- Accuracy. DEFAULT VALUE - Might want to introduce some configuration input here 
   ...
   VLIDORT_FixIn%Cont%TS_VLIDORT_ACCURACY = 0.001d0 !jbak 0.0001d0
...
!    -- One Level only,  TOA or BOA output level

   VLIDORT_FixIn%UserVal%TS_N_USER_LEVELS = 1           ! One level
   VLIDORT_ModIn%MUserVal%TS_USER_LEVELS  = zero        ! initialize

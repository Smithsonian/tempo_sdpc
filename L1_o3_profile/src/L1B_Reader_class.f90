...
MODULE L1B_Reader_class
  ...
  TYPE, PUBLIC :: L1B_block_type
    ...
    ! ====================================         ============    ================
    ! Geolocation fields
    REAL (KIND = 8),    DIMENSION(:),   POINTER :: Time          ! Time
    REAL (KIND = 4),    DIMENSION(:),   POINTER :: SecInDay      ! SecondsInDay
    REAL (KIND = 4),    DIMENSION(:),   POINTER :: ScLat         ! SpacecraftLatitude
    REAL (KIND = 4),    DIMENSION(:),   POINTER :: ScLon         ! SpacecraftLongitude
    REAL (KIND = 4),    DIMENSION(:),   POINTER :: ScAlt         ! SpacecraftAltitude
    REAL (KIND = 4),    DIMENSION(:),   POINTER :: SolElevation  ! SolarElevation
    REAL (KIND = 4),    DIMENSION(:),   POINTER :: SolAzimuth    ! SolarAzimuth
    REAL (KIND = 4),    DIMENSION(:),   POINTER :: SolElevMin    ! SolarElevationMinimum
    REAL (KIND = 4),    DIMENSION(:),   POINTER :: SolElevMax    ! SolarElevationMaximum
    REAL (KIND = 4),    DIMENSION(:),   POINTER :: SolAziMin     ! SolarAzimuthMinimum
    REAL (KIND = 4),    DIMENSION(:),   POINTER :: SolAziMax     ! SolarAzimuthMaximum
    ...
    INTEGER (KIND = 2), DIMENSION(:,:), POINTER :: TerrainHeight ! TerrainHeight
    ...
    ! Data fields
    INTEGER (KIND = 2), DIMENSION(:,:,:), POINTER :: RadMantissa ! RadianceMantissa or
    ! IrradianceMantissa or
    ! SignalMantissa
    INTEGER (KIND = 2), DIMENSION(:,:,:), POINTER :: RadPrecision! RadiancePrecision or
    ! IrradiancePrecision or
    ! SignalPrecision
    INTEGER (KIND = 1), DIMENSION(:,:,:), POINTER :: RadExponent ! RadianceExponent or
    ! IrradianceExponent or
    ! SignalExponent
    INTEGER (KIND = 2), DIMENSION(:,:,:), POINTER :: PQFlag      ! PixelQualityFlags
    INTEGER (KIND = 2), DIMENSION(:,:),   POINTER :: PFlag      ! PixelQualityFlags
    REAL (KIND = 4),    DIMENSION(:,:,:), POINTER :: WavelenCoef ! WavelengthCoefficient
    REAL (KIND = 4),    DIMENSION(:,:,:), POINTER :: WavelenPrec  
    ! WavelengthCoefficientPrecision
    INTEGER (KIND = 2), DIMENSION(:), POINTER :: WavelenRefCol! WavelengthReferenceColumn
    REAL (KIND = 4),  DIMENSION(:,:), POINTER :: SmPixRad     ! SmallPixelRadiance or
    ! SmallPixelIrradiance or
    ! SmallPixelSignal
    REAL (KIND = 4),  DIMENSION(:,:), POINTER :: SmPixWavelen ! SmallPixelWavelength

    INTEGER (KIND = 1), DIMENSION(:), POINTER :: MeasClass    ! MeasurementClass
    INTEGER (KIND = 1), DIMENSION(:), POINTER :: Config       ! InstrumentConfigurationId
    INTEGER (KIND = 2), DIMENSION(:), POINTER :: MQFlag       ! MeasurementQualityFlags
    INTEGER (KIND = 1), DIMENSION(:), POINTER :: NumSmPixCol  ! NumberSmallPixelColumns
    INTEGER (KIND = 1), DIMENSION(:), POINTER :: NumSmPix     ! NumberSmallPixelColumns_cal
    INTEGER (KIND = 1), DIMENSION(:), POINTER :: ExposureType ! ExposureType
    REAL (KIND = 4),    DIMENSION(:), POINTER :: MasterClkPer ! MasterClockPeriod
    INTEGER (KIND = 2), DIMENSION(:), POINTER :: CalSet       ! CalibrationSettings
    REAL (KIND = 4),    DIMENSION(:), POINTER :: ExpTime      ! ExposureTime
    REAL (KIND = 4),    DIMENSION(:), POINTER :: ReadTime     ! ReadoutTime
    INTEGER (KIND = 2), DIMENSION(:), POINTER :: SmPixCol     ! SmallPixelColumn
    INTEGER (KIND = 2), DIMENSION(:), POINTER :: GSC1         ! GainSwitchingColumn1
    INTEGER (KIND = 2), DIMENSION(:), POINTER :: GSC2         ! GainSwitchingColumn2
    INTEGER (KIND = 2), DIMENSION(:), POINTER :: GSC3         ! GainSwitchingColumn3
    INTEGER (KIND = 1), DIMENSION(:), POINTER :: GC1          ! GainCode1
    INTEGER (KIND = 1), DIMENSION(:), POINTER :: GC2          ! GainCode2
    INTEGER (KIND = 1), DIMENSION(:), POINTER :: GC3          ! GainCode3
    INTEGER (KIND = 1), DIMENSION(:), POINTER :: GC4          ! GainCode4
    INTEGER (KIND = 1), DIMENSION(:), POINTER :: DSGC         ! DSGainCode
    INTEGER (KIND = 1), DIMENSION(:), POINTER :: LSLABF ! LowerStrayLightAreaBinningFactor
    INTEGER (KIND = 1), DIMENSION(:), POINTER :: USLABF ! UpperStrayLightAreaBinningFactor
    INTEGER (KIND = 1), DIMENSION(:), POINTER :: LDABF        ! LowerDarkAreaBinningFactor
    INTEGER (KIND = 1), DIMENSION(:), POINTER :: UDABF        ! UpperDarkAreaBinningFactor
    INTEGER (KIND = 2), DIMENSION(:), POINTER :: SR1          ! SkipRows1
    INTEGER (KIND = 2), DIMENSION(:), POINTER :: SR2          ! SkipRows2
    INTEGER (KIND = 2), DIMENSION(:), POINTER :: SR3          ! SkipRows3
    INTEGER (KIND = 2), DIMENSION(:), POINTER :: SR4          ! SkipRows4
    REAL (KIND = 4),    DIMENSION(:), POINTER :: DetcTemp     ! DetectorTemperature
    REAL (KIND = 4),    DIMENSION(:), POINTER :: OptBenchTemp ! OpticalBenchTemperature
    INTEGER (KIND = 1), DIMENSION(:), POINTER :: ImBinFact    ! ImageBinningFactor
    INTEGER (KIND = 2), DIMENSION(:), POINTER :: BinImgRows   ! BinnedImageRows
    INTEGER (KIND = 2), DIMENSION(:), POINTER :: StopColumn   ! StopColumn
...
!    INTEGER (KIND = 4) :: dum
    ...
    IF (ASSOCIATED(this%Time)) NULLIFY(this%Time)
    ...
    IF(ierr .NE. 0) THEN
      ...
      GOTO 999
    ENDIF
    ALLOCATE (this%NumSmPixCol(this%NumTimes), STAT = ierr) !tpk  (this%NumSmPixCol(this%nLine), STAT = ierr)
    IF(ierr .NE. 0) THEN
      message = "NumSmPixCol"
      GOTO 999
    ENDIF
    ...
    ! For PGI 6.0 compiler
    IF (this%NumTimesSmallPixel .gt. 0) then
...
1001 continue
  ...
  FUNCTION L1Br_getGEOline(this, iLine, Time_k, SecondsInDay_k, &
    ...
    INTEGER (KIND=i1), PARAMETER :: szoom_mode = 4_i1!, global_mode = 8_i1
    INTEGER (KIND = 4), INTENT(IN) :: iLine
    REAL (KIND = 8), OPTIONAL, INTENT(OUT) :: Time_k
...
!        Signal_k(1:Nwl_l, 1:this%nXtrack) =  &
!             this%RadMantissa(il:ih,1:this%nXtrack,j) &
!             * 10.0**this%RadExponent(il:ih,1:this%nXtrack,j)
        DO q = il, ih
          DO i = 1, this%nXtrack
...
!        SignalPrecision_k(1:Nwl_l, 1:this%nXtrack) = &
!             this%RadPrecision(il:ih,1:this%nXtrack,j) &
!             * 10.0**this%RadExponent(il:ih,1:this%nXtrack,j)
        DO q = il, ih
          DO i = 1, this%nXtrack
        ...
        ELSE
          ...
          ELSE  
            ...
            IF(PRESENT(SignalPrecision_k)) THEN
              rad_il = this%RadPrecision(il,i,j) * 10.0**this%RadExponent(il,i,j)
              rad_ih = this%RadPrecision(ih,i,j) * 10.0**this%RadExponent(ih,i,j)
              SignalPrecision_k(iw,i) = MAX(rad_il*1.0, rad_ih*1.0)
            ENDIF
...
!    j = i + nPix
  ...
  FUNCTION L1Bga_NumTimes( he4filename, swathname, NumTimesSmallPixel ) &
       RESULT( NumTimes )
    USE hdfeos4_parameters
    INCLUDE 'PGS_OMI_1900.f'  !defines the L1B PGS error codes
    CHARACTER (LEN = *), INTENT(IN) :: he4filename, swathname
    INTEGER (KIND = 4), INTENT(OUT), OPTIONAL :: NumTimesSmallPixel
    INTEGER (KIND = 4) :: NumTimes
    INTEGER (KIND = 4) :: swfid, swid, status, ierr 
    CHARACTER (LEN = MAX_NAME_LENGTH ) :: msg
    ...
    IF( swfid /=  -1) THEN
      ...
      IF( swid /= -1 ) THEN
        ...
        IF( status == -1 ) THEN
          WRITE(msg,'(A)') "Get swath attribute NumTimes"// &
               "failed from "//TRIM(swathname)//","  // &
               TRIM( he4filename )
          ierr = OMI_SMF_setmsg( OMI_E_INPUT,  msg, &
               "L1Bga_EarthSunDistance", zero )
        ...
        IF( PRESENT(NumTimesSmallPixel) ) THEN
          status = swrdattr( swid, "NumTimesSmallPixel", NumTimesSmallPixel )
          IF( status == -1 ) THEN
            WRITE(msg,'(A)') "Get swath attribute NumTimesSmallPixel"// &
                 "failed from "//TRIM(swathname)//","  // &
                 TRIM( he4filename )
            ierr = OMI_SMF_setmsg( OMI_E_INPUT,  msg, &
                 "L1Bga_EarthSunDistance", zero )
            NumTimesSmallPixel = -1
          ENDIF
        ENDIF
        status = swdetach(swid)
      ENDIF

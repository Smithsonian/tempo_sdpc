...
contains
  ...
  !
  ! This has been swtiched to a forward model, but R is for elastic and 
  ! I is for inelastic
  ...
  SUBROUTINE raman(refdir, nulo, nuhi, nline, nz, T, sca, cossza, R, tran, &
    ...
    REAL (KIND=dp), DIMENSION(nulo+maxpos:nuhi-maxpos)     :: RaylP, nr, &
         Raylro, N2so, O2so, I_tot, R_tot, e, Raylcsec
    REAL (KIND=dp), DIMENSION(nulo+maxpos:nuhi-maxpos, nz) :: I, N2sumin, &
         O2sumin, diff
    ...
    CHARACTER (len=maxchlen) :: N2En, N2pos, O2En, O2pos, O2EnfZ, O2J, &
         O2JfZ, O2PT, N2PT 
    ...
    IF (first) THEN
      ...
      OPEN(49, file = O2EnfZ, status='old') 
      do k = 0, O2maxJ
    ...
    DO nu = fidx, lidx 
      ...
      Raylcsec(nu) = 32.0d0 * (REAL(nu))**4 * pi3 * &
           (nr(nu))**2 * (1.0d0 + e(nu) / 4.5d0) / 3.d0 
    ...
    RaylP(fidx:lidx) = 1.d0 + Raylro(fidx:lidx) + &
         (1.d0 - Raylro(fidx:lidx) ) * temp
    RaylP(fidx:lidx) = 3.d0 / (4.d0 + 2.d0 * Raylro(fidx:lidx) ) * &
         RaylP(fidx:lidx)        
    ...
    DO nu = fidx, lidx
      ...
      DO j = 0, 2 * N2Jmax-3
        ...
        N2sumin(nu, 1:nz) = N2sumin(nu, 1:nz) + &
             R(nu + N2shift(j), 1:nz) / R(nu, 1:nz) * N2csec(j) * &
      ...
      DO k = 0, 2 * O2max-7           
        ...
        O2sumin(nu, 1:nz) = O2sumin(nu, 1:nz) + &
             R(nu + O2shift(k), 1:nz) / R(nu, 1:nz) * O2csec(k) &
             * gammaO2(nu + O2shift(k)) ** 2 * temp2
      ENDDO

      diff(nu, 1:nz) = N2mix * (N2sumin(nu, 1:nz) - N2so(nu)) + &
           O2mix * (O2sumin(nu, 1:nz) - O2so(nu))
      I(nu, 1:nz) = R(nu, 1:nz) * ( 1.0 + phasefnc * &
           diff(nu, 1:nz) / (Raylcsec(nu) * RaylP(nu)))
    ENDDO
    !ENDDO                     ! ends nz loop

...
MODULE OMSAO_omidata_module
  ...
  REAL (KIND=dp), DIMENSION(mswath) :: reduce_ubnd, reduce_lbnd, retubnd, &
       retlbnd
  ...
  !REAL (KIND=dp), DIMENSION (nxtrack_max, 0:nlines_max-1, n_max_fitpars) :: &
  !     omi_fitvar
  real (kind=dp), dimension(:,:,:), allocatable :: omi_fitvar

  ! -----------------------
  ! Arrays for OMI L1b data
  ! -----------------------
  !  REAL (KIND=r4), DIMENSION (0:nlines_max-1) :: omi_auraalt, &
  !             omi_auralon, omi_auralat
  ...
  !       omi_latitude, omi_longitude, omi_szenith, omi_sazimuth, &
  !       omi_vzenith, omi_vazimuth, omi_eaza, omi_esca
  !REAL    (KIND=r4), DIMENSION (nwavel_max,nxtrack_max,0:nlines_max-1) ::  &
  !     omi_radiance_spec, omi_radiance_prec, omi_radiance_wavl
  real (kind=4), dimension(:,:,:), allocatable :: omi_radiance_spec, &
       omi_radiance_prec, omi_radiance_wavl
  REAL (KIND=r8), DIMENSION(nxtrack_max) :: omi_solnorm
  REAL (KIND=r8), DIMENSION(nxtrack_max, 0:nlines_max-1) :: omi_radnorm
  !INTEGER (KIND=i2), DIMENSION (nwavel_max,nxtrack_max,0:nlines_max-1) :: &
  !     omi_radiance_qflg, radwind
  integer (kind=2), dimension(:,:,:), allocatable ::omi_radiance_qflg, radwind
  ...
  REAL (KIND=r4), DIMENSION (spc_idx, max_ring_pts, nxtrack_max) :: &
       omi_solspec_ring
  INTEGER, DIMENSION (nxtrack_max) :: solring_lin, solring_uin, omi_nsolring,&
       omi_solring_ndiv
  REAL (KIND=r4), DIMENSION (spc_idx, mrefl, nxtrack_max) :: omi_solspecr
  REAL (KIND=r4), DIMENSION (spc_idx, mrefl, nxtrack_max, 0:nlines_max-1) :: &
       omi_specr
  ...
  ! ---------------------------------------------------------------------------
  ! Correlations with main output product. Due to a bug in HDF-EOS5 routines
  ! (non-TLCF implementation), STRING fields can't be written to file directly.
  ...
  ! --------------------------------------------------------
  !REAL (KIND=r8), DIMENSION (o3_t1_idx:o3_t3_idx, nxtrack_max,0:nlines_max-1) :: omi_o3_amount, omi_o3_uncert
  ...
  ! --------------------------------
  CHARACTER (LEN=maxchlen), DIMENSION(mswath) :: omi_radiance_swathname, &
       omi_irradiance_swathname
  ...
  REAL (KIND=dp), DIMENSION (max_fit_pts, nxtrack_max) ::  &
       omi_slitwav_sol=0.0, omi_sswav_sol=0.0, &
       omi_slitwav_rad=0.0, omi_sswav_rad=0.0
  real (kind=dp), dimension(:,:,:,:), allocatable :: omi_solslitfit, &
       omi_radslitfit
  !  REAL (KIND=dp), DIMENSION (max_fit_pts, max_calfit_idx, 2, nxtrack_max) :: &
  !       omi_solslitfit=0.0, omi_radslitfit=0.0!, &
  !       !omi_solwavfit=0.0, omi_radwavfit=0.0
  INTEGER, DIMENSION (nxtrack_max) :: omi_nslit_sol, omi_nwavcal_sol, &
       omi_nslit_rad, omi_nwavcal_rad
  REAL (KIND=dp), DIMENSION(maxwin,max_calfit_idx,2, nxtrack_max) :: &
       omi_solwinfit, omi_radwinfit

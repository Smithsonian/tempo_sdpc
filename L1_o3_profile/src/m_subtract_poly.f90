...
!    EXTERNAL poly_specfit
    ...
    REAL (KIND=dp), DIMENSION (:,:), ALLOCATABLE :: dfda
    ...
    nlower = MINVAL ( MINLOC ( locwvl(1:npts), MASK=(locwvl(1:npts) >= &
         locwvl(ll_rad)-1.0_dp) ) )
    nupper = MAXVAL ( MAXLOC ( locwvl(1:npts), MASK=(locwvl(1:npts) <= &
         locwvl(lu_rad)+1.0_dp) ) )
    nfitted = nupper - nlower + 1
    ! Find average position over fitted region
    locavg = SUM ( locwvl(1+nlower-1:nfitted+nlower-1) ) / &
         REAL ( nfitted, KIND=dp )
    ...
    allocate (f (nfitted), dfda(nfitted, poly_order))
    DO i = 1, max_rs_idx
      IF ( i /= solar_idx ) THEN
        ! ===============================================================
        ...
        CALL elsunc ( par, poly_order, nfitted, nfitted, poly_specfit, &
             elbnd, blow(1:poly_order), bupp(1:poly_order), p, w, exval, &
             f(1:nfitted), dfda(1:nfitted,1:poly_order) )
        chisq = SUM ( f(1:nfitted)**2 ) 
        ! This gives the same CHI**2 as the NR routines
        x(1:npts) = locwvl(1:npts) - locavg

        poly_x(1:npts) = x(1:npts)
        exval = 3
        CALL poly_specfit ( par(1:poly_order), poly_order, poly_y(1:npts), &
             npts, exval, dfda, 0 )
...
!    EXTERNAL poly_specfit
    ...
    REAL (KIND=dp), DIMENSION (poly_order)           :: r!, par, polfunc
    !REAL (KIND=dp), DIMENSION (poly_order,poly_order):: covar

    INTEGER                             :: i, nlower, nupper, nfitted!, j
    REAL (KIND=dp)                      :: locavg!, chisq
    ...
    !     Find limits for polynomial fitting, with ~1 nm overlap
    nlower = MINVAL(MINLOC( locwvl(1:npoints), MASK=(locwvl(1:npoints) >= &
         locwvl(ll_rad)-1.0) ))
    nupper = MAXVAL(MAXLOC( locwvl(1:npoints), MASK=(locwvl(1:npoints) <= &
         locwvl(lu_rad)+1.0) ))
    nfitted = nupper - nlower + 1
    ...
    CALL elsunc ( r, poly_order, nfitted, nfitted, poly_specfit, elbnd, &
         blow(1:poly_order), bupp(1:poly_order), p, w, exval, f(1:nfitted), &
         dfda(1:nfitted,1:poly_order) )
    chisq2 = SUM ( f(1:nfitted)**2 ) 
    ! This gives the same CHI**2 as the NR routines
    ...
    exval = 3
    CALL poly_specfit ( r(1:poly_order), poly_order, poly_y(1:npoints), &
         npoints, exval, dfda, 0 )
    locspec(1:npoints) = locspec(1:npoints) - poly_y(1:npoints)

    RETURN
  END SUBROUTINE subtract_poly_meas

end module m_subtract_poly

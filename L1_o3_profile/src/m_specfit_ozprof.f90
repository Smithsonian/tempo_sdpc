...
contains
  ...
  SUBROUTINE specfit_ozprof (initval, fitcol, dfitcol, rms, exval)
    ...
    CALL make_atm(the_year, the_month, the_day, ndiv, &
         the_cod, the_cfrac, the_ctp, nlay, toz, ps0, pst, &
         atmosprof(:,0:nlay), ozprof(1:nlay), nup2p(0:nlay), &
         sacldscl, errstat)  
    ...
    ! For BrO:  a priori from model fields but with 1.0E-14 error globally 
    !           (enough information)
    ...
    ! For aerosols, a priori is based on model, 
    !    a priori error is assumed as 100%, 
    ...
    IF (initval == 0 .OR. taodfind == 0) fitvar_rad_init(taodind) = &
         tropaod(actawin)
    IF (initval == 0 .OR. twaefind == 0) fitvar_rad_init(twaeind) = &
         tropwaer(actawin)
    ...
    fitvar_rad_init(ecfrind) = the_cfrac
    fitvar_rad_apriori(ecfrind) = the_cfrac
    fitvar_rad_init(ecodind) = the_cod
    fitvar_rad_apriori(ecodind) = the_cod
    fitvar_rad_init(ectpind) = the_ctp
    fitvar_rad_apriori(ectpind) = the_ctp
    IF (so2zfind > 0) THEN
      fitvar_rad_init(so2zind) = fitvar_rad_init_saved(so2zind)
      lo_radbnd(so2zind) = the_surfalt
      up_radbnd(so2zind) = 30.
      fitvar_rad_apriori(so2zind)  = fitvar_rad_init(so2zind)
    ENDIF

    ! For surface pressure
    fitvar_rad_init(sprsind) = ps0
    fitvar_rad_apriori(sprsind) = ps0 
    IF (sprsfind > 0) THEN
      lo_radbnd(sprsind) = atmosprof(1, nlay) - &
           (atmosprof(1, nlay)-atmosprof(1, nlay-1)) * 0.5
      up_radbnd(sprsind) = atmosprof(1, nlay) + &
           (atmosprof(1, nlay)-atmosprof(1, nlay-1)) * 0.5
    ENDIF
    ...
    !xliu, 08/29/05 scaling a priori for layers below clouds to avoid 
    !smoothing even for full cloudy conditions
...
!                sa(i, i) = 0.0 ! added for update v2
    ...
    IF (exval <= 0) THEN
      DO i = 1, numwin
        fitvar_rad_init(osind(i, 1:maxoth)) = 0.0
        fitvar_rad_init(shind(i, 1:maxoth)) = 0.0
      ENDDO
    ENDIF
    ...
    DO i = 1, nf
      stderr(i) = SQRT(covar(i, i))
      stderr1(i) = SQRT(ncovar(i, i))
    END DO
    ...
    DO is = 1, 2
      IF (is == 1 ) THEN
        ...
        covar_sav(ozf_fidx:ozf_lidx, ozf_fidx:ozf_lidx) = &
             covar(ozf_fidx:ozf_lidx, ozf_fidx:ozf_lidx)
      ELSE
        IF (.NOT. use_oe) EXIT
        ozprof_std_sav(ozf_fidx:ozf_lidx) = ozprof_nstd(ozf_fidx:ozf_lidx)
        covar_sav(ozf_fidx:ozf_lidx, ozf_fidx:ozf_lidx) = &
             ncovar(ozf_fidx:ozf_lidx, ozf_fidx:ozf_lidx)
      ENDIF
      ...
      DO i = start_layer, nsfc
        IF (ozprof_std_sav(i) > 0.0) THEN
          ...
          DO j = start_layer, i - 1
            dfitcol(1, is) = dfitcol(1, is) + &
                 2.0 * ozprof_std_sav(i) * ozprof_std_sav(j) * &
                 covar_sav(k1+i, k1+j) / SQRT(covar_sav(k1+i, k1+i) * &
                 covar_sav(k1+j, k1+j))
          END DO
        ENDIF
      ENDDO
      ...
      IF (ntp > 0) THEN     
        ...
        DO i = ntp + 1, nsfc
          IF (ozprof_std_sav(i) > 0.0) THEN
            ...
            DO j = ntp + 1, i - 1
              dfitcol(3, is) = dfitcol(3, is) + 2.0 * ozprof_std_sav(i) * &
                   ozprof_std_sav(j) * covar_sav(k1+i, k1+j) / &
                   SQRT(covar_sav(k1+i, k1+i) * covar_sav(k1+j, k1+j))

              !print *, ozprof_std_sav(i), ozprof_std_sav(j), &
              !sqrt(covar_sav(k1+i, k1+i)), sqrt(covar_sav(k1+j, k1+j))
            ENDDO
          ENDIF
        ENDDO
      ENDIF
    ...
    IF (use_logstate) THEN
      DO i = ozf_fidx, ozf_lidx
        ozprof_apstd(j) = SQRT(sa(i, i))*ozprof_ap(j)
        j = j + 1
      ENDDO
    ELSE
      DO i = ozf_fidx, ozf_lidx
        ozprof_apstd(j) = SQRT(sa(i, i))
        j = j + 1
      ENDDO
    ENDIF

    IF (nsfc < nlay) THEN
      ozprof(nsfc+1:nlay) = -99.99
      ozprof_ap(nsfc+1:nlay) = -99.99
    ENDIF
    ...
    DO k = 1, ngas
      ...
      IF (i > 0) THEN
        ...
        fitvar_rad_std(j) = tracegas(k, 5)
        fitvar_rad_nstd(j) = tracegas(k, 6)

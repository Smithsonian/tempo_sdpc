...
contains
  ...
  SUBROUTINE angle_sat2toa ( earth_curv, ers2_alt, atm_hght, n_arr_pix, &
    ...
    USE OMSAO_parameters_module,   ONLY: rad2deg, pi!, deg2rad
    IMPLICIT NONE
    ...
    ! ----------------------------------
    earth_zen = ASIN( SIN (zen) * (earth_curv+ers2_alt) / &
         (earth_curv+atm_hght) )
    ...
    ! ------------------------------------------------------------
    locazm = COS(earth_zen) * SIN(zen0) * COS(relazm) - &
         SIN(ABS(earth_zen)) * COS(zen0)

    ! -----------------------------------
    ! Step 3: Correction of zenith angles
    ! -----------------------------------
    zen0 = ACOS(COS(zen0) * COS(earth_zen) + SIN(ABS(earth_zen)) * &
         SIN(zen0) * COS(relazm) )
    zen  = zen + earth_zen
    ...
    ! aza: average between A and C (flip flop at B for nadir and 
    ! back-scan pixels)
  ...
  SUBROUTINE omi_angle_sat2toa ( n_arr_pix, zen0, zen, relazm, sza, vza, &
       aza, sca )
    ...
    ELSE 
      ! FIXME 
      ! zen0(3) can be >=pi/2, in which case LOG fails
      ! This brute force solution limits the maximum value
      ! but there must be a better way to do this
      ! zenith values >90 degrees are nonsensical anyway
      if (zen0(3) >= 0.99999999d0*pi/2.0d0) then
          zen0(3) = 0.99999999d0*pi/2.0d0
      endif
      sza = ACOS( 1.0 / ((LOG(TAN(zen0(1)/2.0+pi/4.0)) - &
           LOG(TAN(zen0(3)/2.0+pi/4.0))) / (zen0(1)-zen0(3)))) 
    ENDIF
    ...
    ! The correct relative azimuthmal angle for radiative transfer model
    aza = 180.0 - aza
  ...
  SUBROUTINE adjust_angle (thght, zen0, zen, relazm, sca)
    ...
    ! compute scattering angle for polarization correction
    relazm = pi - relazm
    sca    = 180.0 - ACOS(COS(zen0) * COS(zen) + SIN(zen0) * SIN(zen) * &
         COS(relazm))  * rad2deg
  ...
  SUBROUTINE scia_angle_sat2toa ( earth_curv, ers2_alt, atm_hght, &
    ...
    ! Step 1: Compute Earth center angle
    ! ----------------------------------
    earth_zen = ASIN( SIN (zen) * (earth_curv + toa_height) / &
         (earth_curv + atm_hght) )
    earth_zen = earth_zen - zen
  ...
  SUBROUTINE gome2_angle_sat2toa ( earth_curv, ers2_alt, atm_hght, &
    ...
    ! CRN: The angles given for GOME-2 are referenced to h=0.0 km, 
    ! so corrections are not needed to put at the surface coordinate. 
    ! This subroutine needs to be modified if atm_hght used is not equal to
    ! zero.

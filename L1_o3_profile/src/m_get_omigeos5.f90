...
MODULE  m_get_omigeos5
  ...
  USE OMSAO_omidata_module,  ONLY : nxtrack_max, ntimes_max, &
                                    zoom_mode , zoom_p1
  USE OMSAO_variables_module, ONLY :coadd_uv2, l2_geos5_filename,nxbin,nybin,&
                                    offset_line, ncoadd
  ...
  TYPE, PUBLIC :: GEOS5_BLock
    ...
    REAL (KIND=r4),DIMENSION (:,:,:),POINTER :: dp, ts
  ...
  SUBROUTINE read_geos5 (nt,nx,nl,  pge_error_status)   
    ...
    LOGICAL :: file_exist
    !----------
    INTEGER, DIMENSION(ngl) :: ord ! convert top-down to down-top
    ...
    IF (first) THEN
      ...
      ALLOCATE (geos5%dp(ngl,nxtrack_max,0:ntimes_max-1))  
      ALLOCATE (geos5%ts(0:ngl,nxtrack_max,0:ntimes_max-1))  
   ...
   GEOS5%dp    (1:ngl, 1:nbx, 0:nl-1) = 0.0
   GEOS5%ts    (1:ngl, 1:nbx, 0:nl-1) = 0.0
   ORD (1:ngl) = (/(k,k=ngl,1, -1 )/) 
   DO ix = 1, nbx                      ! index after co-adding
    DO iy = 0 , nl-1
       ...
       Do j = iix, iix + nbin -1
          ...
          GEOS5%dp(1:ngl,ix,iy)=GEOS5%dp(1:ngl,ix,iy)+ LOG(dp(1:ngl,j,k))
       ...
       GEOS5%dp(1:ngl,ix,iy)=EXP(GEOS5%dp(ORD, ix, iy)/cc)*0.01 ! convert from pa to HPa
       GEOS5%ts(1:ngl,ix,iy)=GEOS5%ts(ORD,ix,iy)/cc
       GEOS5%lat(ix,iy)     =GEOS5%lat(ix,iy)/cc
       GEOS5%lon(ix,iy)     =GEOS5%lon(ix,iy)/cc       
       GEOS5%ps(0,ix,iy)    =GEOS5%spres(ix,iy)  

       Do k = 1, ngl 
           GEOS5%ps(k,ix,iy) = GEOS5%ps(k-1,ix,iy) - GEOS5%dp(k,ix,iy)  
       ENDDO
    ENDDO
   ENDDO
   IF (zoom_mode) THEN 
      ...
      GEOS5%dp(1:ngl,1+xoff:nbx+xoff,0:nl-1)=GEOS5%dp(1:ngl,1:nbx,0:nl-1)

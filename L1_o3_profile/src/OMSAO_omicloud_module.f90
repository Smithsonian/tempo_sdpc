MODULE OMSAO_omicloud_module
  ...
  USE OMSAO_omidata_module,    ONLY: nxtrack_max, nlines_max, ntimes_max, &
      nfxtrack, zoom_mode, zoom_p1
  USE OMSAO_variables_module,  ONLY: l2_cld_filename, coadd_uv2, nxbin, nybin,offset_line, ncoadd
  ...
  public read_omicldrr_clouds, read_omicldo2_clouds
  private fill_in_ctp
...
CONTAINS
  ...
  SUBROUTINE read_omicldrr_clouds (nt, nx, nl, pge_error_status)
    ...
    use he5_input_tools, only: he5_init_input_file
    ...
    ! Local variables
    ! -----------------------
    INTEGER   (KIND=i4)      :: locerrstat, swath_id, swath_file_id, nt_loc, &
         nx_loc, ix, iix, i, ii, j, k, nline, nbx, nbin, sline, eline, nx1, &
         xoff, xoff0
    ...
    integer (kind=i2), dimension(nt) :: tmp_byte_num
    integer (kind=i2), dimension(nt, 0:nbit-1) :: tmp_bit_num
    ...
    CALL he5_init_input_file ( l2_cld_filename, swath_name,  swath_id, &
         swath_file_id, nt_loc, nx_loc, locerrstat)

    IF ( locerrstat /= pge_errstat_ok ) THEN
      estat = OMI_SMF_setmsg (omsao_e_open_l2cld_file, modulename, &
           'OML2 Cloud access failed', 0) 
      STOP 1
    END IF

    ! check if the dimensions are consistent with OMI L1B data
    IF (nt_loc /= nt .OR. nx_loc /= nx) THEN
!      WRITE(www_lun, *) 'OMICLDRR: ', nt_loc, nx_loc
!      WRITE(www_lun, *) 'OMIL2 : ', nt, nx
!      WRITE(www_lun, '(A)') &
! Messages being written to file that is not the log - move to stdout
      print *, 'OMICLDRR: ', nt_loc, nx_loc
      print *, 'OMIL2 : ', nt, nx
      print *, &
           'Inconsistent dimensions between OMIL1B and OMIL2!!!'
    ...
    IF ( locerrstat /= pge_errstat_ok ) THEN
      estat = OMI_SMF_setmsg (omsao_e_open_l2cld_file, modulename, &
           'OML2 Cloud CFR access failed', 0)
      STOP 1
    END IF
    ...
    IF ( locerrstat /= pge_errstat_ok ) THEN
      estat = OMI_SMF_setmsg (omsao_e_open_l2cld_file, modulename, &
           'OML2 Cloud CTP access failed', 0)
      STOP 1
    END IF
    ...
    IF ( locerrstat /= pge_errstat_ok ) THEN
      estat = OMI_SMF_setmsg (omsao_e_open_l2cld_file, modulename, &
           'OML2 Cloud Quality Flags access failed', 0)
      STOP 1
    END IF
    ...
    cfr(1:nx1,0:nt-1) = cfr(1:nx1,0:nt-1) * OMIL2_clouds%CFRscale + &
         OMIL2_clouds%CFRoffset
    ctp(1:nx1,0:nt-1) = ctp(1:nx1,0:nt-1) * OMIL2_clouds%CTPscale + &
         OMIL2_clouds%CTPoffset
    ai(1:nx1, 0:nt-1) = 0.0
    flgbits = 0
    sline = offset_line
    nline = nl * nybin
    ...
    DO ix = 1, nx1
      ...
      tmp_byte_num=qflag(ix, 0:nt-1)
!      CALL convert_2bytes_to_16bits (nbit, nt, qflag(ix, 0:nt-1), &
!           flgbits(ix, 0:nt-1, 0:nbit-1))
      CALL convert_2bytes_to_16bits (nbit, nt, tmp_byte_num, &
           tmp_bit_num)
!      tmp_bit_num=flgbits(ix, 0:nt-1, 0:nbit-1)
      flgbits(ix, 0:nt-1, 0:nbit-1)=tmp_bit_num
    ENDDO
    ...
    qflag(1:nx1, :)  = flgbits(1:nx1, :, 0) + flgbits(1:nx1, :, 1) + &
         flgbits(1:nx1, :, 2) + flgbits(1:nx1, :, 3) + &
         flgbits(1:nx1, :, 4) + flgbits(1:nx1, :, 6) + &
         flgbits(1:nx1, :, 7) + flgbits(1:nx1, :, 9) + &
         flgbits(1:nx1, :, 11) + flgbits(1:nx1, :, 13) + &
        ...
        DO j = iix, iix + nbin - 1
          DO k = ii, ii + nybin - 1
            ...
            IF ( ctp(j, k) > 0.0 .AND. cfr(j, k) >= 0.0 ) THEN                 
              OMIL2_clouds%ctp(ix, i) = &
                   OMIL2_clouds%ctp(ix, i) + LOG(ctp(j, k)) * cfr(j, k)
    ...
    IF (zoom_mode) THEN
      ...
      OMIL2_clouds%cfr   (1+xoff:nbx+xoff, 0:nl-1) = &
           OMIL2_clouds%cfr   (1:nbx, 0:nl-1)
      OMIL2_clouds%ctp   (1+xoff:nbx+xoff, 0:nl-1) = &
           OMIL2_clouds%ctp   (1:nbx, 0:nl-1)
      OMIL2_clouds%ai    (1+xoff:nbx+xoff, 0:nl-1) = &
           OMIL2_clouds%ai    (1:nbx, 0:nl-1)
      OMIL2_clouds%qflags(1+xoff:nbx+xoff, 0:nl-1) = &
           OMIL2_clouds%qflags(1:nbx, 0:nl-1)
    ENDIF
  ...
  SUBROUTINE read_omicldo2_clouds (nt, nx, nl, pge_error_status)
    ...
    ! Local variables
    ! -----------------------
    INTEGER   (KIND=i4)  :: locerrstat, swath_id, swath_file_id, &
         nt_loc, nx_loc, ix, iix, i, ii, j, k, nline, nbx, nbin, sline, &
         eline, nx1, xoff, xoff0
    REAL      (KIND=r4)  :: scale_cfr, offset_cfr, missval_cfr, scale_ctp, &
         offset_ctp, missval_ctp, scfr, scfr1, scfr0, tmpsum
    CHARACTER (LEN=maxchlen) :: swath_name
    ...
    !  WRITE(www_lun, '(A)') 'Inconsistent dimensions between OMIL1B and OMIL2!!!'
      ...
      DO i = 0, nl - 1        
        ...
        DO j = iix, iix + nbin - 1
          DO k = ii, ii + nybin - 1
            ...
            IF ( ctp(j, k) > 0.0 .AND. cfr(j, k) >= 0.0 ) THEN                 
              OMIL2_clouds%ctp(ix, i) = OMIL2_clouds%ctp(ix, i) + &
                   LOG(ctp(j, k)) * cfr(j, k)
              tmpsum = tmpsum + LOG(ctp(j, k)) 
              scfr = scfr + cfr(j, k)
              scfr0 = scfr0 + 1.0
    ...
    DO ix = 1, nx      
      ...
      DO WHILE ( i <= lidx )
        IF (ctp(ix, i) == 0.0 ) THEN
          ...
          IF (dis <= 12) THEN
            DO j = sidx + 1, eidx - 1
              frac = 1.0 - REAL((j - sidx), KIND=r4) / dis

!>
module m_adj_gome_data
  use m_angle_sat2toa

  public adj_solar_data, adj_earthshine_data
  private !adj_rad_sig, calc_view_geometry,
...
contains
  ...
  SUBROUTINE adj_earthshine_data ( error )
    ...
    USE OMSAO_parameters_module,   ONLY : normweight!, rad2deg, pi, deg2rad, &
    !downweight
    USE OMSAO_variables_module,    ONLY : curr_rad_spec, n_rad_wvl, &
         the_sza_atm, the_vza_atm, the_aza_atm, use_meas_sig, the_month, &
         the_day, the_lon, the_lat, numwin, nradpix, the_surfalt, &
         actspec_rad, database, refidx, radwavcal_freq, yn_varyslit, &
...
!!$  ENDIF
    ...
    IF ( (gome_curpix == 1) .OR. MOD(gome_curpix, radwavcal_freq) == 0 ) THEN
      ...
      IF (yn_varyslit) THEN
        IF (slit_rad) CALL radiance_fit_vary( n_gome_radpts, &
             curr_rad_spec(wvl_idx:sig_idx, 1:n_gome_radpts), error)

        IF (.NOT. slit_rad .OR. wavcal_sol) CALL radiance_wavcal_vary ( &

module m_cloud_pres_mod
  ...
  real (KIND=8), parameter :: sat_rad=6.5e13!6e13! this one worked! 
  !old value was 5.5e13
  !7e13 too high
  ...
  real (KIND=8) :: ixd, icd, np, np0, nc, ir, irc, irco
  real (KIND=8) :: nt, j, l, nt_o, j_o, l_o
  real (KIND=8) :: refl_oc
  integer :: ngood
  ...
  real (KIND=8), dimension(:), allocatable :: waves, y_obs, y_obs1, y_calc, &
       y_frac, y_calc_sh, y_calc_squeeze, sflx, sflx1, wave_diff
  ...
  real (KIND=8), dimension(:), allocatable :: ycalc, rad_tot, &
       rad_tot_noring, ref_rad, ref_ring
  real (KIND=8), dimension(:), allocatable :: rad_clr, rad_cld, &
       ring_cld2,ring_clr, ring_cld, jacob_dummy, jacob_rad, fit_rad

...
contains
  ...
  subroutine write_output_data(outfile, swathname)
    ...
    character(len=255) :: dimsw, logmsg
    ...
    !-------------------------------------------------
    write(logmsg,"(A28, A)") 'write_output_data: creating ',trim(outfile)
    call tell_log(1,logmsg)
    swfid = he5_swopen (outfile, HE5_ACC_TRUNC)
    write(logmsg,"(A25,I12)") 'write_output_data: swfid ', swfid
    call tell_log(1,logmsg)

    ! Create the swath.
    !-----------------
    swid = he5_swcreate (swfid, swathname)
    write(logmsg,"(A25,I12)") 'write_output_data: swid  ', swfid
    call tell_log(1,logmsg)

    ! Write the dimensions
    !---------------------
    write(logmsg,"(A38,I6,A9,I6)") 'write_output_data: dimensions, nTimes ', &
         nTimes, ' nXtrack ',nXtrack
    call tell_log(1,logmsg)
    status = he5_swdefdim(swid,nTimesstr,INT(nTimes, KIND=C_LONG))
    write(logmsg,"(I12,2X,A22)") status, 'after define nTimesstr'
    call tell_log(1,logmsg)
    status = he5_swdefdim(swid,nXtrackstr,INT(nXtrack, KIND=C_LONG))
    write(logmsg,"(I12,2X,A23)") status, 'after define nXtrackstr'
    call tell_log(1,logmsg)
    if (write_resid) then
      nWaveRes=size(wave_resid)
      status = he5_swdefdim(swid,nWavestr,INT(nWaveRes, KIND=C_LONG))
      write(logmsg,"(I12,2X,A21)") status, 'after define nWavestr'
      call tell_log(1,logmsg)
    endif
    dimsw = trim(nWavestr)
    ...
    status = put_data (swid, "Time", &
         dims1, time, misval_r8, "Time at Start of Scan (s, TAI93)", "s", &
         geo=.true.)
    if (write_geom) then
    ...
    status = put_data (swid, "TerrainHeight", &
         dims2, terr_height, misval_i2, "Terrain Height", "m", &
         geo=.true.,offset=(/0,0/))

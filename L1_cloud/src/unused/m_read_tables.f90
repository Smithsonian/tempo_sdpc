...
contains
  ...
  subroutine read_tables (rc)
    ...
    !
    ! !USES:
    !  read_tables reads precomputed Ring effect table needed for cloud
    ...
    !  07Aug14   O'Sullivan added variable descriptions
    !
    !EOP
    !-------------------------------------------------------------------------
    integer :: i, j, k, l, m
      ...
      do j=1, nphi
        do k=1, nscan
          do l=1, ntheta
            do m=1, nwave
              if (first) then
                read(lun, err=100) k1bar(m), nba(i,m), sba(i,m) ! when using solar flux, only depends on wavelength
              endif

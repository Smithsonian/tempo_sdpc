...
contains
  subroutine read_references (rc)
    ...
    use m_vars, ONLY: nwav, nscanpos, input_data_path, refl_Ref, & 
         psurf_ref, sza_ref, satz_ref, az_Ref, lat_ref, lon_ref, & 
         thresh_file, filename, ex, iprt, ntheta, theta, nscan, & 
         scan, nphi, phi, npres, pres, &
         reference_spec, reference_wave, reference_ring, reference_rad
    use m_LUN_set
    ...
    ! !INTERFACE:
    !
    ! !USES: read_references reads precomputed references spectra needed 
    !        for cloud pressures
    !
    ! !INPUT PARAMETERS:
    !
